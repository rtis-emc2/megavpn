...
func newClient(baseURL, token, statePath string) *client {
	return newClientWithHTTP(baseURL, token, statePath, &http.Client{Timeout: 10 * time.Second})
}
...
func TestNextJobVerifiesSignedResponse(t *testing.T) {
	...
	c.http = httpDoerFunc(func(req *http.Request) (*http.Response, error) {
		...
		return signedAgentTestResponse(t, req, "agent-token", http.StatusOK, body), nil
	})
...
func TestNextJobRejectsTamperedSignedResponse(t *testing.T) {
	...
	c.http = httpDoerFunc(func(req *http.Request) (*http.Response, error) {
		...
		resp := signedAgentTestResponse(t, req, "agent-token", http.StatusOK, signedBody)
...
func TestNextJobAcceptsSignedNoContentResponse(t *testing.T) {
	...
	c := newClient("https://control.example", "agent-token", "")
	c.http = httpDoerFunc(func(req *http.Request) (*http.Response, error) {
		return signedAgentTestResponse(t, req, "agent-token", http.StatusNoContent, nil), nil
	})
...
func TestNextJobRejectsReplayedSignedResponse(t *testing.T) {
	...
	c.http = httpDoerFunc(func(req *http.Request) (*http.Response, error) {
		body := []byte(`{"id":"job-1","type":"instance.apply","payload":{}}` + "\n")
		resp := signedAgentTestResponse(t, req, "agent-token", http.StatusOK, body)
...
func signedAgentTestResponse(t *testing.T, req *http.Request, token string, status int, body []byte) *http.Response {
	timestamp := strconv.FormatInt(time.Now().UTC().Unix(), 10)
	nonce := mustAgentClientNonce(t)
...
func mustAgentClientNonce(t *testing.T) string {
	t.Helper()
