...
func (c client) emergencyCleanupNode(ctx context.Context, j job, st agentState) (string, map[string]any) {
	...
	cleanupScope, err := validateEmergencyCleanupJobPayload(j.Payload, includeAgent)
	if err != nil {
...
func validateEmergencyCleanupJobPayload(payload map[string]any, includeAgent bool) (string, error) {
	cleanupScope := stringify(payload["cleanup_scope"])
	switch cleanupScope {
	case domain.NodeEmergencyCleanupScopeServicesOnly, domain.NodeEmergencyCleanupScopeFullNode:
	default:
		return "", fmt.Errorf("cleanup_scope must be services_only or full_node")
	}
	if includeAgent && cleanupScope != domain.NodeEmergencyCleanupScopeFullNode {
		return "", fmt.Errorf("include_agent requires cleanup_scope full_node")
	}
	reason := stringify(payload["reason"])
	if err := domain.ValidateNodeEmergencyCleanupReason(reason); err != nil {
		return "", fmt.Errorf("emergency cleanup reason is invalid: %w", err)
	}
	planVersion, ok := emergencyCleanupPlanVersion(payload["cleanup_plan_version"])
	if !ok {
		return "", fmt.Errorf("cleanup_plan_version is required")
	}
	if planVersion != domain.NodeEmergencyCleanupPlanVersion {
		return "", fmt.Errorf("cleanup_plan_version is unsupported")
	}
	switch payload["instances"].(type) {
	case []any, []map[string]any:
	default:
		return "", fmt.Errorf("instances cleanup plan must be an array")
	}
	return cleanupScope, nil
}
