...
func TestValidateBackhaulManagedFilePolicyAllowsGeneratedUnitAndScript(t *testing.T) {
	...
	stopScript := managedFileSpec{
		Path:    "/etc/megavpn/backhaul/link/ingress-stop.sh",
		Content: "#!/usr/bin/env sh\nset -eu\n\n# Generated by megavpn-agent. Do not edit manually.\nif command -v nft >/dev/null 2>&1; then\nfi\n/usr/bin/wg-quick down '/etc/megavpn/backhaul/link/wg.conf' >/dev/null 2>&1 || true\n",
		Mode:    "0700",
	}
	payload := map[string]any{
		"systemd_unit": "megavpn-backhaul-link.service",
		"files": []any{
			map[string]any{"path": startScript.Path, "mode": startScript.Mode, "content": startScript.Content},
			map[string]any{"path": stopScript.Path, "mode": stopScript.Mode, "content": stopScript.Content},
		},
	}
	...
	if err := validateBackhaulManagedFilePolicy(startScript, payload); err != nil {
		t.Fatalf("expected generated script to be allowed: %v", err)
	}
}

func TestValidateBackhaulManagedFilePolicyRejectsExtraExecDirective(t *testing.T) {
	t.Parallel()

	payload := map[string]any{"systemd_unit": "megavpn-backhaul-link.service"}
	unit := managedFileSpec{
		Path:    "/etc/systemd/system/megavpn-backhaul-link.service",
		Content: "[Service]\nExecStart=/usr/bin/wg-quick up /etc/megavpn/backhaul/link/wg.conf\nExecStartPre=/usr/bin/id\n",
		Mode:    "0644",
	}
	if err := validateBackhaulManagedFilePolicy(unit, payload); err == nil {
