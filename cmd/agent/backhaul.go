...
func validateBackhaulManagedFilePolicy(file managedFileSpec, payload map[string]any) error {
	...
	if strings.HasPrefix(path, "/etc/systemd/system/") {
		...
		if !isAllowedBackhaulServiceFile(file.Content, allowedBackhaulScriptsFromPayload(payload)) {
...
func allowedBackhaulScriptsFromPayload(payload map[string]any) map[string]string {
	files, err := decodeManagedFiles(payload["files"])
	if err != nil {
		return nil
	}
...
func isAllowedBackhaulServiceFile(content string, allowedScripts map[string]string) bool {
	if strings.Contains(content, "/bin/sh") || strings.Contains(content, " -c ") || strings.Contains(content, "\x00") || strings.Contains(content, "\\\n") {
		return false
	}
	...
	for _, line := range strings.Split(content, "\n") {
		...
		if !isAllowedBackhaulExecCommand(cmd, allowedScripts) {
			return false
		}
	}
	return seenStart
...
func isAllowedBackhaulExecCommand(cmd string, allowedScripts map[string]string) bool {
	cmd = strings.TrimSpace(cmd)
	...
	case strings.HasPrefix(cmd, defaultBackhaulRoot):
		...
		script := cleanAbsPath(strings.Trim(fields[0], "'\""))
		if script == "" || !isSafeBackhaulManagedPath(script) || !strings.HasSuffix(script, ".sh") {
			return false
		}
		content, ok := allowedScripts[script]
		return ok && isAllowedBackhaulScriptFile(script, content)
	default:
		return false
	}
}
...
func isAllowedBackhaulScriptFile(path, content string) bool {
	...
	for _, line := range strings.Split(content, "\n") {
		if !isAllowedBackhaulScriptLine(strings.TrimSpace(line)) {
			return false
		}
	}
	return true
...
func isAllowedBackhaulScriptLine(line string) bool {
	if line == "" ||
		line == "#!/usr/bin/env sh" ||
		line == "set -eu" ||
		line == "# Generated by megavpn-agent. Do not edit manually." ||
		line == "sysctl -w net.ipv4.ip_forward=1 >/dev/null" ||
		line == "if command -v nft >/dev/null 2>&1; then" ||
		line == "if command -v ip >/dev/null 2>&1; then" ||
		line == "fi" ||
		line == "for handle in $handles; do" ||
		line == "done" {
		return true
	}
	switch {
	case strings.HasPrefix(line, "/usr/bin/wg-quick up "), strings.HasPrefix(line, "/usr/bin/wg-quick down "):
		return backhaulExecContainsManagedPath(line)
	case strings.HasPrefix(line, "/usr/sbin/openvpn --daemon "):
		return strings.Contains(line, " --config ") && backhaulExecContainsManagedPath(line)
	case strings.HasPrefix(line, "/usr/bin/env xray run -config "):
		return backhaulExecContainsManagedPath(line)
	case strings.HasPrefix(line, "test ! -f ") && strings.Contains(line, " || kill $(cat ") && strings.HasSuffix(line, ") >/dev/null 2>&1 || true"):
		return backhaulExecContainsManagedPath(line) || containsSafeBackhaulRuntimePath(line)
	case strings.HasPrefix(line, "nft list table ip megavpn_backhaul "), strings.HasPrefix(line, "nft list chain ip megavpn_backhaul postrouting "):
		return true
	case strings.HasPrefix(line, "if ! nft list chain ip megavpn_backhaul postrouting | grep -Fq "):
		return strings.Contains(line, "megavpn:backhaul:")
	case strings.HasPrefix(line, "nft add rule ip megavpn_backhaul postrouting "):
		return (strings.Contains(line, " iifname ") || strings.Contains(line, " oifname ")) &&
			strings.Contains(line, " masquerade comment ") &&
			strings.Contains(line, "megavpn:backhaul:")
	case strings.HasPrefix(line, "handles=$(nft -a list chain ip megavpn_backhaul postrouting "):
		return strings.Contains(line, "awk -v c=") && strings.Contains(line, "megavpn:backhaul:")
	case strings.HasPrefix(line, "nft delete rule ip megavpn_backhaul postrouting handle "):
		return strings.HasSuffix(line, " >/dev/null 2>&1 || true")
	case strings.HasPrefix(line, "while ip rule delete from "):
		return strings.Contains(line, " table ") && strings.HasSuffix(line, " >/dev/null 2>&1; do :; done")
	case strings.HasPrefix(line, "ip route flush table "):
		return strings.HasSuffix(line, " >/dev/null 2>&1 || true")
	default:
		return false
	}
...
func containsSafeBackhaulRuntimePath(line string) bool {
	for _, field := range strings.Fields(line) {
		path := cleanAbsPath(strings.Trim(field, "'\"()"))
		if path == "" {
			continue
		}
		if strings.HasPrefix(path, "/run/megavpn-backhaul/") && !hasUnsafePathToken(path) {
			return true
		}
	}
	return false
...
func removeManagedPath(path string, recursive bool) (bool, error) {
	...
	if path == "" {
		return false, fmt.Errorf("managed path must be absolute")
	}
