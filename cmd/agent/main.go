...
func main() {
	...
	client, err := newConfiguredClient(st.ControlPlaneURL, st.AgentToken, cfg.Agent.StatePath, cfg.Agent)
	if err != nil {
		log.Error("agent transport configuration invalid", "error", err)
		os.Exit(1)
	}
	runControlLoop(root, log, cfg.Agent.PollInterval, client, st)
}
