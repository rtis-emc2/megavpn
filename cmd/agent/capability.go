...
func (c client) installCapability(ctx context.Context, j job, st agentState) (string, map[string]any) {
	...
	case "nginx":
		...
		if strategy == "manual_present" {
			res := verifyNginx(ctx)
			c.recordCapabilityInventorySync(ctx, st.NodeID, res)
	...
	case "shadowsocks":
		...
		if res["ok"] != true && hasBinaryRepositoryInstallPayload(j, "binary_repository_fallback") {
			...
			c.recordCapabilityInventorySync(ctx, st.NodeID, fallback)
...
func (c client) recordCapabilityInventorySync(ctx context.Context, nodeID string, result map[string]any) {
	if result == nil {
		return
	}
	if err := c.submitInventory(ctx, nodeID, "inventory", collectInventory()); err != nil {
		result["inventory_sync_status"] = "failed"
		result["inventory_sync_error"] = truncate(err.Error(), 1000)
		return
	}
	result["inventory_sync_status"] = "succeeded"
}
