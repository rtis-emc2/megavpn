...
func ensureXrayInstanceDriverState(ctx context.Context, store driverStateStore, instanceID string) error {
	...
	if groupChanged, err := ensureXrayInstanceDefaultEgress(ctx, store, instance, spec); err != nil {
		return err
...
func ensureXrayInstanceDefaultEgress(ctx context.Context, store driverStateStore, instance domain.Instance, spec map[string]any) (bool, error) {
	changed := normalizeLegacyXrayVLESSGroupEgress(spec)
	mode := strings.ToLower(firstNonEmpty(
		stringify(spec["egress_mode"]),
		stringify(spec["xray_egress_mode"]),
		stringify(spec["vless_egress_mode"]),
		stringify(nestedMapLocal(spec, "xray_egress")["mode"]),
		"auto",
	))
	if mode == "local" || mode == "direct" || mode == "local_breakout" {
		spec["xray_egress"] = map[string]any{
			"mode":            "local_breakout",
	...
	egressNodeID := firstNonEmpty(
		stringify(spec["egress_node_id"]),
		stringify(spec["xray_egress_node_id"]),
		stringify(spec["vless_egress_node_id"]),
		stringify(nestedMapLocal(spec, "xray_egress")["egress_node_id"]),
		stringify(nestedMapLocal(spec, "xray_egress")["node_id"]),
	)
...
func normalizeLegacyXrayVLESSGroupEgress(spec map[string]any) bool {
	...
	for idx, item := range groups {
		...
		if firstNonEmpty(stringify(group["label"])) == "Default egress" {
			group["label"] = "Default access"
			changed = true
		}
		if strings.EqualFold(firstNonEmpty(stringify(group["egress_mode"])), "auto") {
			delete(group, "egress_mode")
			changed = true
		}
		if strings.EqualFold(firstNonEmpty(stringify(group["egress_resolved_by"])), "worker:xray-egress") {
...
func ensureXrayVLESSGroupEgress(ctx context.Context, store driverStateStore, instance domain.Instance, spec map[string]any) (bool, error) {
	groups, key, changed := xrayMutableVLESSGroupsLocal(spec)
	for idx, item := range groups {
		group, _ := item.(map[string]any)
		if group == nil {
...
func ensureOpenVPNInstanceAndClientState(ctx context.Context, store driverStateStore, record *domain.ProvisioningAccess) error {
...
func ensureOpenVPNPlatformPKIRoot(ctx context.Context, store driverStateStore, profile string) (domain.PlatformServicePKIRoot, error) {
...
func ensureShadowsocksInstanceDriverState(ctx context.Context, store driverStateStore, instanceID string) error {
	instance, err := store.GetInstanceWithSpec(ctx, instanceID)
	if err != nil {
		return err
	}
...
func ensureHTTPProxyInstanceDriverState(ctx context.Context, store driverStateStore, instanceID string) error {
	instance, err := store.GetInstanceWithSpec(ctx, instanceID)
	if err != nil {
		return err
	}
...
func ensureMTProtoInstanceDriverState(ctx context.Context, store driverStateStore, instanceID string) error {
	instance, err := store.GetInstanceWithSpec(ctx, instanceID)
	if err != nil {
		return err
	}
...
func ensureWireGuardInstanceDriverState(ctx context.Context, store driverStateStore, instanceID string) error {
	instance, err := store.GetInstanceWithSpec(ctx, instanceID)
	if err != nil {
		return err
	}
...
func ensureIPSecL2TPInstanceDriverState(ctx context.Context, store driverStateStore, record *domain.ProvisioningAccess) (map[string]string, error) {
	instance, err := store.GetInstanceWithSpec(ctx, record.Instance.ID)
	if err != nil {
		return nil, err
	}
