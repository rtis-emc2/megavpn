...
Базовая версия кода: RTIS MegaVPN 7.1.0.2
...
## 19. Release 7.1.0.2 Closure

Цель релиза `7.1.0.2` - сделать managed firewall понятным перед расширением
production rollout. Runtime enforcement уже был реализован; проблема была в
том, что UI и документация не показывали оператору связь между address lists,
rules, policies, apply jobs и node state.

Закрыто в этом релизе:

- Firewall Overview теперь показывает pipeline от каталога до node apply:
  address lists -> rules -> policy -> apply job -> node state.
- Firewall Rules теперь показывает compact rule anatomy:
  priority -> chain -> match -> action -> apply mode.
- Firewall documentation содержит Mermaid model diagram и явную таблицу
  default baseline правил для production node.
- Дальнейший development path ставит user traffic accounting с retention минимум
  180 дней первым пунктом до включения production traffic collection.
- Web asset cache keys, release banners и release review artifact подняты до
  `7.1.0.2`.

Database migration, public API contract и node nftables runtime behavior не
менялись. Default firewall seed и agent renderer остаются под существующими
integration и agent tests.

## 20. Immediate Next Actions

1. Спроектировать user traffic accounting с retention минимум 180 дней: event
   schema, aggregation, partitioning, privacy boundary, RBAC и audit export.
