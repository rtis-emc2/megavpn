...
Базовая версия кода: RTIS MegaVPN 7.1.0.11
...
## 19. Release 7.1.0.11 Closure

Цель релиза `7.1.0.11` - сделать видимыми missing expected Traffic Accounting
...
- Expected collector rows строятся из active/enabled managed
  Xray/WireGuard/OpenVPN instances, у которых current revision включает traffic
  accounting, и full-join-ятся с observed retained samples.
...
- Collector freshness thresholds заданы детерминированными server-side
  ...
  validation.
- Regression tests покрывают collector freshness и missing-stream
  classification вместе с retained-dataset filter и CSV coverage из предыдущего
  инкремента.
