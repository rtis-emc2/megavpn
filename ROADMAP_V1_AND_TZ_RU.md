...
Базовая версия кода: RTIS MegaVPN 7.1.0.9
...
## 19. Release 7.1.0.9 Closure

Цель релиза `7.1.0.9` - сделать Traffic Accounting filters authoritative на
сервере для overview summary, recent rows и CSV export без расширения privacy
boundary. Система по-прежнему хранит агрегированные byte counters, а не
payload, URLs, DNS queries или per-destination browsing history.

Закрыто в этом релизе:

- `/api/v1/traffic/accounting` теперь принимает те же retained-dataset filters,
  что и CSV export: `from`, `to`, `protocol`, `client_id`, `node_id` и `limit`.
- Overview summary и recent rows фильтруются в PostgreSQL через тот же
  validated predicate builder, что используется CSV export.
- Oversized overview limits clamp-ятся к read maximum вместо fallback в меньший
  implicit value.
- Traffic Accounting UI перезагружает overview с backend-а при изменении
  filters и использует те же выбранные значения для CSV export.
- Regression tests покрывают shared SQL predicates, UUID validation, limit
  clamping, export filter request parsing, inverted date ranges и CSV header
  uniqueness.
- Web asset cache keys, release banners и release review artifact подняты до
  `7.1.0.9`.

Agent runtime collector behavior и database schema не менялись. Существующее
permission `traffic.read` по-прежнему защищает overview и CSV export.
