...
Базовая версия кода: RTIS MegaVPN 7.1.0.7
...
## 19. Release 7.1.0.7 Closure

Цель релиза `7.1.0.7` - сделать retention behavior для audited
traffic-accounting видимым без расширения privacy boundary. Система по-прежнему
хранит агрегированные byte counters, а не payload, URLs, DNS queries или
per-destination browsing history.

Закрыто в этом релизе:

- Traffic-accounting overview summary теперь содержит `retention_cutoff`,
  `expired_sample_count`, `prune_batch_size`, `prune_batches_per_ingest` и
  `max_prune_per_ingest`.
- Traffic Accounting UI показывает retention cutoff, expired cleanup backlog и
  bounded prune budget рядом с существующими counters по samples/clients/nodes.
- Store использует общий helper retention cutoff для overview, export и prune
  paths, чтобы API behavior оставался согласованным.
- Regression tests покрывают retention-cutoff helper и prune-budget
  calculation.
- Text wrapping для summary cards усилен, чтобы длинные cutoff values не
  раздвигали layout.
- Web asset cache keys, release banners и release review artifact подняты до
  `7.1.0.7`.

Ingest/export contracts, agent runtime collector behavior и database schema не
менялись. Read-only overview response получил operational retention metadata
под существующим permission `traffic.read`.
