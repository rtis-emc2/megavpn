...
Базовая версия кода: RTIS MegaVPN 7.1.0.8
...
## 19. Release 7.1.0.8 Closure

Цель релиза `7.1.0.8` - сделать traffic-accounting audit export workflow
удобным из operator UI без расширения privacy boundary. Система по-прежнему
хранит агрегированные byte counters, а не payload, URLs, DNS queries или
per-destination browsing history.

Закрыто в этом релизе:

- Traffic Accounting UI теперь содержит export filters для `from`, `to`,
  `protocol`, `client_id`, `node_id` и `limit`.
- Recent-row preview использует те же выбранные оператором filters до CSV
  export.
- Export filter state хранится в session scope, чтобы navigation не сбрасывал
  audit handoff context.
- Regression tests покрывают export filter request parsing, inverted date ranges
  и CSV header uniqueness.
- Export filter form использует compact desktop layout и возвращается к
  существующему one-column responsive form behavior на малых экранах.
- Web asset cache keys, release banners и release review artifact подняты до
  `7.1.0.8`.

API contract, agent runtime collector behavior и database schema не менялись.
UI теперь использует существующие read-only export filters под существующим
