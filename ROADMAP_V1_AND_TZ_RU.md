...
Базовая версия кода: RTIS MegaVPN 7.0.1.4
...
`7.0.1.4` является boundary-релизом между feature expansion и production hardening. Цель этой ветки - не добавлять хаотично новые сервисы, а стабилизировать три слоя:
...
## 2.2 Product Roadmap After 7.0.1.4
...
## 11. Release 7.0.1.4 Closure

Цель релиза `7.0.1.4`: закрыть следующий production-hardening слой для
service-pack/runtime apply: OpenVPN full-tunnel NAT contract, traffic
camouflage ingress profiles и rollback managed files при failed validation/apply.

Зафиксировано в этом релизе:

- Default OpenVPN packs стали full-tunnel baseline: push `redirect-gateway`,
  публичные DNS, managed IP forwarding и nftables masquerade от client pool.
- Agent network policy renderer материализует `nat_rules`, очищает старые
  managed NAT rules по instance comment prefix и возвращает `nat_count`.
- Xray+Nginx WebSocket/gRPC camouflage packs используют loopback Xray backend и
  обязательный fallback website вместо скрытого `example.com` default.
- API/UI create-from-pack принимает `camouflage_path`, `fallback_upstream_url`,
  `fallback_host_header`, `fallback_sni`; Host/SNI могут выводиться из fallback
  URL.
- Nginx/API validation режет небезопасные `location_path`, fallback directive
  values, credentials и не-HTTP(S) fallback URL.
- Agent `instance.apply` делает snapshot managed files и восстанавливает старые
  файлы или удаляет новые при failed validation, network-policy или systemd
  apply.
- `scripts/self-test.sh` и `scripts/release-gate.sh` используют временный
  `GOMODCACHE`, если он не задан явно, чтобы локальный corrupted module cache не
  ломал release evidence.

Что сознательно остается после текущего release baseline:

- Прогнать service-pack smoke matrix на реальной disposable node: Xray Reality,
  Xray+Nginx gRPC, Xray WebSocket camouflage с fallback website, OpenVPN
  TCP/UDP full tunnel, WireGuard, Shadowsocks и Backhaul.
