...
Базовая версия кода: RTIS MegaVPN 7.1.0.4
...
## 19. Release 7.1.0.4 Closure

Цель релиза `7.1.0.4` - расширить audited user traffic accounting с
Xray/VLESS на managed WireGuard и OpenVPN без выхода за privacy boundary:
система хранит агрегированные byte counters, а не payload, URLs, DNS queries
или per-destination browsing history.

Закрыто в этом релизе:

- Agent собирает Xray Stats API, WireGuard `wg show <interface> transfer` и
  OpenVPN status-file counters как delta buckets.
- Managed OpenVPN configs рендерят `status-version 2`, managed `status` path и
  managed `ifconfig-pool-persist` path.
- Managed WireGuard configs рендерят non-secret attribution comments для peer
  diagnostics.
- Control Plane ingest сопоставляет OpenVPN/WireGuard samples с
  `service_accesses` через существующую metadata identity.
- Default service-pack templates включают `traffic_accounting_enabled` для
  managed OpenVPN/WireGuard, а migration `000015` обновляет существующие default
  templates.
- Web asset cache keys, release banners и release review artifact подняты до
  `7.1.0.4`.

Public API contract не менялся. Database migration затрагивает только default
service-pack templates. Existing instances нужно re-apply после upgrade, чтобы
node получила managed OpenVPN status path и WireGuard attribution comments.
...
1. Проверить traffic accounting на живых nodes: Xray Stats API,
   ...
   export audit trail, partitioning/retention под реальную cardinality.
