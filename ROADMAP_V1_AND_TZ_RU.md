...
Базовая версия кода: RTIS MegaVPN 7.1.0.10
...
## 19. Release 7.1.0.10 Closure

Цель релиза `7.1.0.10` - сделать live Traffic Accounting validation наблюдаемой
без расширения privacy boundary. Система по-прежнему хранит агрегированные byte
counters, а не payload, URLs, DNS queries или per-destination browsing history.
...
- Collector status включает active/degraded/inactive freshness, last report
  timestamp, last bucket timestamp, sample count, client count и aggregate
  byte/flow counters для выбранного retained dataset.
- Traffic Accounting UI теперь показывает отдельную Collector status table
...
- Collector freshness thresholds заданы детерминированными server-side
  constants: active внутри normal reporting window, degraded при задержке и
  inactive, когда stream молчит достаточно долго для operator validation.
- Regression tests покрывают collector freshness classification вместе с
  retained-dataset filter и CSV coverage из предыдущего инкремента.
...
менялись. Существующее permission `traffic.read` защищает overview, collector
status и CSV export.
