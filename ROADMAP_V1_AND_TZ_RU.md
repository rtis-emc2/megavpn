...
Базовая версия кода: RTIS MegaVPN 7.0.1.10
...
`7.0.1.10` является boundary-релизом между feature expansion и production hardening. Цель этой ветки - не добавлять хаотично новые сервисы, а стабилизировать три слоя:
...
## 2.2 Product Roadmap After 7.0.1.10
...
## 11. Release 7.0.1.10 Closure

Цель релиза `7.0.1.10`: закрыть visual/layout regression после Firewall UI
hardening, чтобы все основные вкладки снова были предсказуемыми на desktop и
mobile перед следующим функциональным инкрементом.

Зафиксировано в этом релизе:

- Backhaul rows получили shrink-safe grid tracks и больше не клипуют правую
  колонку действий на desktop.
- Instances toolbar теперь укладывается в доступную ширину shell без обрезания
  фильтров и action cluster.
- Node Map pending GeoIP rows корректно сжимаются внутри правой detail-колонки.
- Revisions page больше не расширяет `body` горизонтально на mobile `390px`.
- Проведен browser smoke всех 17 основных вкладок на desktop и `390x844`
  mobile: body-level horizontal overflow, off-viewport controls, clipped
  controls и console errors не обнаружены.
- Обновлены release version, web asset cache-busting и release/security review
  artifact `SECURITY_REVIEW_7.0.1.10.md`.
