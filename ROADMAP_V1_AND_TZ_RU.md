...
Базовая версия кода: RTIS MegaVPN 7.0.1.9
...
`7.0.1.9` является boundary-релизом между feature expansion и production hardening. Цель этой ветки - не добавлять хаотично новые сервисы, а стабилизировать три слоя:
...
## 2.2 Product Roadmap After 7.0.1.9
...
## 11. Release 7.0.1.9 Closure

Цель релиза `7.0.1.9`: закрыть следующий hardening-инкремент для
traffic-camouflage ingress profiles и repeatable service-pack smoke.

Зафиксировано в этом релизе:

- Nginx renderer валидирует fallback upstream как абсолютный `http://` или
  `https://` URL и режет credentials, relative URL и unsupported schemes.
- WebSocket camouflage pack перешел с raw `location_extra_lines` на
  структурированный флаг `websocket_upgrade`; renderer сам добавляет Upgrade /
  Connection headers, long read/send timeouts и `proxy_buffering off`.
- `scripts/service-pack-smoke.sh` принимает `MEGAVPN_FALLBACK_UPSTREAM_URL`,
  `MEGAVPN_CAMOUFLAGE_PATH`, `MEGAVPN_FALLBACK_HOST_HEADER` и
  `MEGAVPN_FALLBACK_SNI`; single-pack smoke fail-fast падает без fallback, а
  matrix smoke честно помечает camouflage packs как `SKIPPED`.
- VLESS provisioning artifacts и VLESS subscription regression tests
  подтверждают, что camouflage profile публикует Nginx endpoint `:443` с
  public WebSocket path и не выдает backend port `7080`.
- Сохранена модель `7.0.1.6`: Xray backend слушает `127.0.0.1`, Nginx
  проксирует только скрытый VLESS/gRPC path, а обычный web-трафик уходит на
  явно заданный fallback website.
