...
Базовая версия кода: RTIS MegaVPN 7.1.0.6
...
## 19. Release 7.1.0.6 Closure

Цель релиза `7.1.0.6` - усилить retention и query paths для audited
traffic-accounting без расширения privacy boundary. Система по-прежнему хранит
агрегированные byte counters, а не payload, URLs, DNS queries или
per-destination browsing history.

Закрыто в этом релизе:

- Retention cleanup теперь идет bounded batches на ingest вместо одного
  unbounded delete по всем expired samples.
- Overview и export queries продолжают применять 180-day retention cutoff, даже
  если physical cleanup постепенно удаляет старый backlog.
- PostgreSQL migration `000016_traffic_accounting_query_indexes` добавляет
  indexes для recent-sample ordering и common export filters по client, node и
  protocol.
- Regression tests покрывают bounded prune query shape и cleanup budget.
- Web asset cache keys, release banners и release review artifact подняты до
  `7.1.0.6`.

Public API и agent runtime collector behavior не менялись. Database migration
добавляет только indexes; retention semantics остаются 180 дней с bounded
physical pruning.
