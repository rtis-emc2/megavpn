...
Базовая версия кода: RTIS MegaVPN 7.0.1.11
...
`7.0.1.11` является boundary-релизом между feature expansion и production hardening. Цель этой ветки - не добавлять хаотично новые сервисы, а стабилизировать три слоя:
...
## 2.2 Product Roadmap After 7.0.1.11
...
## 11. Release 7.0.1.11 Closure

Цель релиза `7.0.1.11`: убрать неоднозначность в UI создания Backhaul-связи,
где operator видел один driver сверху и набор protocol-like профилей снизу.
Теперь форма явно разделяет active primary transport и standby/internal
transport profiles.

Зафиксировано в этом релизе:

- Backhaul create modal переименовал `Preferred driver` в
  `Primary transport driver`.
- Primary driver автоматически включается в `Transport profiles to create`,
  получает метку `primary`, не снимается checkbox-ом и первым попадает в
  `drivers` payload.
- Дополнительные выбранные профили маркируются как `standby`, невыбранные
  остаются `available`.
- Backhaul details modal показывает `Primary transport`, а не legacy
  `Preferred driver`.
- `docs/BACKHAUL.md`, English/Russian user guides и security review объясняют,
  что это internal node-to-node transports, а не клиентские VPN-протоколы.
- Проведен browser smoke Backhaul create modal на desktop и `390x844` mobile:
  body/modal horizontal overflow не обнаружен, dynamic primary selection
  работает корректно.
- Обновлены release version, web asset cache-busting и release/security review
  artifact `SECURITY_REVIEW_7.0.1.11.md`.
...
- Следующим functional инкрементом нужно продолжить traffic-camouflage ingress
  case: reverse proxy до целевого сайта, маскировка VLESS под него и repeatable
  service-pack smoke.
