...
Базовая версия кода: RTIS MegaVPN 7.1.0.5
...
## 19. Release 7.1.0.5 Closure

Цель релиза `7.1.0.5` - сделать audited traffic-accounting data выгружаемым для
operator handoff без расширения privacy boundary. Система по-прежнему хранит
агрегированные byte counters, а не payload, URLs, DNS queries или
per-destination browsing history.

Закрыто в этом релизе:

- Добавлен `GET /api/v1/traffic/accounting/export` для CSV audit export под
  существующим permission `traffic.read`.
- Export filters поддерживают `limit`, `from`, `to`, `client_id`, `node_id` и
  `protocol`, с server-side caps и retention cutoff enforcement.
- CSV responses отдаются с `Cache-Control: no-store` и attachment disposition.
- Traffic Accounting UI получил action `Export CSV` для operator handoff.
- Regression tests покрывают export time parsing и CSV row serialization.
- Web asset cache keys, release banners и release review artifact подняты до
  `7.1.0.5`.

Database migration и agent runtime behavior не менялись. Public API surface
получил read-only export endpoint, защищенный существующим traffic accounting
RBAC permission.
...
1. Проверить traffic accounting на живых nodes: Xray Stats API,
   ...
   partitioning/retention под реальную cardinality.
