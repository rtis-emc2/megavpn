...
- Audit.
- Agent endpoints: register, heartbeat, inventory, next job, job result.

### 3.2 Database / Domain Model

Реализованы миграции `000001`-`000022`.
...
- `instance_runtime_states` как отдельная нормализованная runtime projection.
- `share_link_events`.
...
- Settings UI and `GET /api/v1/platform/pki-roots` expose platform PKI root inventory without exposing CA private key secret references.
...
- Signed payloads/job signatures отсутствуют.
...
- Нет rollback command.
- Нет normalized runtime state projection.
...
| Job signature validation | Required | Not implemented | Required for v1.0 security baseline |
...
- Implement mTLS for agent transport or formal signed payload alternative.
- Sign jobs and verify job signatures on agent.
...
- Add `instance_runtime_states`.
- Add `instance_health_checks`.
...
|---|---|---|
| mTLS/signature model delayed | Security blocker | Implement before service expansion |
...
## 11. Release 0.6.7.1-alpha Closure

Цель релиза `0.6.7.1-alpha`: закрыть базовую production-структуру перед следующей итерацией `0.6.7.2`.
