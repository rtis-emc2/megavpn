...
Managed backhaul links connect ingress and egress nodes through `/api/v1/backhaul-links` and the Backhaul UI page. WireGuard and OpenVPN UDP/TCP profiles are materialized and activated by `node.backhaul.apply` agent jobs. Operators can run bidirectional `node.backhaul.probe` checks from the UI and managed delete queues `node.backhaul.cleanup` jobs to remove generated units/files from both nodes before the link disappears. Stale `running` cleanup jobs are recovered back to `retrying` when their lease expires, so a dead agent request no longer blocks deletion forever. IPsec/IKEv2/L2TP and Xray/VLESS profiles are written as explicit config/profile files and intentionally require a manual activation stage until routing-loop, edge-TLS and strongSwan profile validation are finalized. See `docs/BACKHAUL.md`.
