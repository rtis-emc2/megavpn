...
Managed backhaul links connect ingress and egress nodes through `/api/v1/backhaul-links` and the Backhaul UI page. The selected internal transport profiles are applied to both nodes by `node.backhaul.apply` agent jobs. WireGuard and OpenVPN UDP/TCP profiles write config, install systemd units, start services and run agent health checks. IPsec/IKEv2/L2TP and Xray/VLESS profiles are written as explicit config/profile files and intentionally remain `materialized` rather than `active` until routing-loop, edge-TLS and strongSwan profile validation are finalized. Operators can run bidirectional `node.backhaul.probe` checks for an active selected transport from the UI, and managed delete queues `node.backhaul.cleanup` jobs to remove generated units/files from both nodes before the link disappears. Stale `running` cleanup jobs are recovered back to `retrying` when their lease expires, so a dead agent request no longer blocks deletion forever. See `docs/BACKHAUL.md`.
