...
Current branch status: expanded service-pack matrix for standalone services, provisioning/artifact/share-link smoke runner for the test server, stricter revision/apply lifecycle with validation markers, rollback baseline and revision diff visibility, certificate lifecycle hardening, job payload validation and deployment baseline stabilization.
