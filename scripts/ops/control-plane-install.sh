...
BACKUP_ROOT="${MEGAVPN_CP_BACKUP_ROOT:-/var/lib/megavpn/backups}"
...
prompt_configuration() {
  ...
  prompt_text WEB_ROOT "Frontend static directory" "$WEB_ROOT" true
...
write_runtime_env() {
  ...
  write_env_line "$tmp" "MEGAVPN_BACKUP_ROOT" "$BACKUP_ROOT"
...
run_migrations() {
  local result
  log "stop API and worker before migrations"
...
main() {
  ...
  ./scripts/install-frontend.sh "$WEB_ROOT"
