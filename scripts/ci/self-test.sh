...
RUN_SYSTEMD_VERIFY="${MEGAVPN_SELF_TEST_RUN_SYSTEMD_VERIFY:-0}"
RUN_NGINX_VERIFY="${MEGAVPN_SELF_TEST_RUN_NGINX_VERIFY:-0}"
...
require_install_frontend_wrapper_smoke() {
  scripts/ci/install-frontend-wrapper-smoke.sh
}
...
require_systemd_verify() {
  if [[ "$RUN_SYSTEMD_VERIFY" != "1" && "$RUN_SYSTEMD_VERIFY" != "true" ]]; then
    skip_check "set MEGAVPN_SELF_TEST_RUN_SYSTEMD_VERIFY=1 on a prepared Linux host"
    return 77
  fi
...
require_nginx_verify() {
  if [[ "$RUN_NGINX_VERIFY" != "1" && "$RUN_NGINX_VERIFY" != "true" ]]; then
    skip_check "set MEGAVPN_SELF_TEST_RUN_NGINX_VERIFY=1 on the target Nginx host"
    return 77
  fi
...
run_check "javascript-syntax" "Operational JavaScript parses under node --check" require_frontend_js_syntax
run_check "frontend-static-guards" "React frontend observes same-origin and injection safety boundaries" require_frontend_static_guards
run_check "frontend-serving-smoke" "React routes, static assets and backend namespaces are served independently" require_frontend_serving_smoke
run_check "install-frontend-wrapper-smoke" "Deployment copies only the verified React build" require_install_frontend_wrapper_smoke
