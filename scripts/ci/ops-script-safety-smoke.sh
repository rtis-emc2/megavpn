...
assert_precedes() {
  ...
  (( before_line < after_line )) || fail "$file must run '$before' before '$after'"
}

cd "$ROOT_DIR"

assert_precedes scripts/ops/control-plane-install.sh \
...
expect_failure "Frontend install into source parent" "Frontend target" \
  env MEGAVPN_FRONTEND_ALLOW_CUSTOM_TARGET=1 scripts/ops/install-frontend.sh "$ROOT_DIR"
