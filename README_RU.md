...
`7.1.0.15` приводит в порядок operator UI для Traffic Accounting: action-кнопки
фильтров больше не растягиваются на всю форму, collection model использует
отдельную responsive-диаграмму, а cache keys Web UI обновлены для предсказуемого
refresh после deployment. Основной фокус:
