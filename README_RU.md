...
`7.1.0.21` добавляет контролируемый lost-node force-retire и сохраняет текущий
Firewall/UI lifecycle baseline: недоступную node теперь можно
убрать из активной модели Control Plane только после явного подтверждения
именем node, а backend отменяет pending/running jobs, удаляет локальные
instance bindings, чистит generated client access artifacts, revoke agent
identity и скрывает связанные backhaul links. Основной фокус:
