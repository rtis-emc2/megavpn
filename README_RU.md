...
`7.1.0.26` переносит VLESS client identity на уровень клиента, а не отдельного
instance metadata. Новый ingress при provisioning наследует уже выданный
client UUID, поэтому стабильный публичный endpoint можно перенести на новые
ноды без перевыпуска профиля пользователю. Миграция backfill-ит registry из
существующих Xray/VLESS service accesses. Предыдущий stabilization baseline
также сохраняет strict SSH host-key bootstrap scanning, защиту формы от
фонового auto-refresh, фильтрацию runtime reports для deleted instances и
очистку orphan runtime state rows. Основной фокус:
