...
`7.1.0.3` - baseline понятности firewall и планирования релиза перед
...
- foundation учета трафика: signed agent ingest API, PostgreSQL aggregate
  storage, retention cleanup 180 дней, RBAC `traffic.read` и отдельная UI
  страница; protocol collectors остаются закрыты до validation на реальных
  nodes;
