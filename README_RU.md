...
`7.1.0.28` делает диагностику активных jobs понятнее в Web UI: agent-handled
jobs вроде `instance.apply` теперь показывают, ждут ли они poll от целевого
node agent, уже забраны агентом или ждут agent result внутри lease. Это убирает
непонятное `n/a` у queued/running runtime apply jobs и показывает node/lock
context в раскрытой строке Jobs. `7.1.0.27` остается SQL-hotfix для VLESS
provisioning: Xray UUID при записи в `client_service_identities` явно кастуется
как text внутри `jsonb_build_object`, поэтому PostgreSQL prepared statements
больше не падают с `could not determine data type of parameter $5`. Baseline
`7.1.0.26` остается целевой моделью: VLESS identity хранится на уровне клиента,
replacement ingress наследует уже выданный client UUID во время provisioning, а
миграция backfill-ит registry из существующих Xray/VLESS service accesses.
Предыдущий stabilization baseline также сохраняет strict SSH host-key bootstrap
scanning, защиту формы от фонового auto-refresh, фильтрацию runtime reports для
deleted instances и очистку orphan runtime state rows. Основной фокус:
...
- VLESS client identity теперь стабилен между Xray/VLESS ingress instances и
  при замене нод: provisioning клиента на новый ingress переиспользует
  client-level UUID даже после удаления старого instance/service access и
  ставит apply, чтобы новый сервер принял существующий клиентский credential;
