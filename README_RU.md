...
Frontend Console `8.0.0` готовится как release candidate в
[docs/releases/8.0.0.md](docs/releases/8.0.0.md). Новая React/Vite-консоль
является кандидатом на primary UI, а предыдущий static UI остается доступен на
`/legacy/` как rollback path на время миграции.
