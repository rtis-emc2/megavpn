...
`7.1.0.20` доводит Firewall operator UI и lifecycle controls: кнопки Firewall
теперь используют один visual system, Apply выделен как high-impact action,
applied state на node визуально отличается, Address group actions имеют
понятную primary/secondary иерархию, node может поставить scoped Firewall
Disable job, SSH bootstrap/reinstall теперь проверяет enforced firewall на node
перед постановкой agent update jobs, а cache keys Web UI обновлены для
предсказуемого refresh после deployment. Основной фокус:
