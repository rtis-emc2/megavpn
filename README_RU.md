...
`7.1.0.18` доводит Firewall operator UI: Firewall panels теперь занимают всю
рабочую ширину, active tabs имеют один понятный selected style, address groups
упрощены до address groups и entries, node apply actions привязаны к строкам
node, а cache keys Web UI обновлены для предсказуемого refresh после
