...
`7.1.0.11` добавляет expected-vs-observed coverage для Traffic Accounting
collectors активных managed instances, сохраняя режим release stabilization.
Основной фокус:
...
- операторская firewall-диаграмма, которая связывает address lists, rules,
  policies, apply jobs и node state в один workflow от каталога до node;
- repair firewall schema для upgraded installations и упрощенный workflow
  address lists без internal identity fields;
...
- видимость retention учета трафика: operator summary показывает cutoff,
  expired-row cleanup backlog и bounded prune budget на ingest;
...
- expected collector coverage сравнивает active traffic-accounting-enabled
  Xray, WireGuard и OpenVPN instances с реально observed sample streams и
  показывает missing или partial retained sample streams в Traffic Accounting UI;
