...
`7.1.0.17` возвращает top-tab workflow на странице Traffic Accounting и
сохраняет упрощенный operator UI: технические поля storage maintenance заменены
на счетчики traffic/client/node/collector, пустой dataset показывает понятную
диагностику, report filters вынесены во вкладку Export, а cache keys Web UI
обновлены для предсказуемого refresh после deployment. Основной фокус:
