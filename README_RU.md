...
`7.1.0.24` усиливает SSH bootstrap без ослабления host-key pinning: в форме
Node management появился управляемый `Scan host key`, который берет текущие
SSH host/port, получает SHA256 fingerprints через privileged API и заполняет
`ssh_host_key_sha256` только после того, как оператор может сверить значение.
Предыдущий stabilization baseline также сохраняет защиту формы от фонового
auto-refresh, фильтрацию runtime reports для deleted instances и очистку orphan
runtime state rows. Основной фокус:
