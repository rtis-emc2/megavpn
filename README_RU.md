...
`7.1.0.22` закрывает gap с очисткой instances на потерянной node: оператор
может force-delete конкретный instance, если node/agent окончательно
недоступен. Backend требует точное подтверждение именем instance, отменяет
pending jobs, которые ссылаются на этот instance, удаляет client access/config
rows и не меняет обычный agent-backed cleanup для живых nodes. Основной фокус:
