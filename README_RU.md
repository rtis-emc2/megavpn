...
`7.1.0.25` исправляет layout таблицы Clients provisioning: колонка Actions
получила стабильную ширину, а сетка кнопок больше не вызывает горизонтальный
overflow и не обрезает действия на широких операторских экранах. Предыдущий
stabilization baseline также сохраняет strict SSH host-key bootstrap scanning,
защиту формы от фонового auto-refresh, фильтрацию runtime reports для deleted
instances и очистку orphan runtime state rows. Основной фокус:
