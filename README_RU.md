...
`7.1.0.27` hotfix-ит VLESS client provisioning после rollout client-level
identity registry: Xray UUID при записи в `client_service_identities` теперь
явно кастуется как text внутри `jsonb_build_object`, поэтому PostgreSQL
prepared statements больше не падают с `could not determine data type of
parameter $5`. Baseline `7.1.0.26` остается целевой моделью: VLESS identity
