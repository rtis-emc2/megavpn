...
`7.1.0.16` упрощает operator UI для Traffic Accounting: технические поля
storage maintenance заменены на счетчики traffic/client/node/collector, пустой
dataset показывает понятную диагностику, action-кнопки фильтров остаются
компактными, а cache keys Web UI обновлены для предсказуемого refresh после
deployment. Основной фокус:
