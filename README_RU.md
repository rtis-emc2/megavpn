...
`7.1.0.29` делает rollout service pack идемпотентным для уже созданных
components: повторный submit на ту же node, endpoint host, service и listen
port переиспользует существующий instance вместо создания suffixed дубля вроде
`xray-ws 2`. API возвращает переиспользованные строки в `existing_instances` и
статус `already_exists`, если весь запрошенный pack уже есть. `7.1.0.28`
остается hotfix-ом Jobs UI diagnostics для ожидания agent-handled
`instance.apply`, а `7.1.0.27` остается SQL-hotfix для VLESS provisioning.
Baseline `7.1.0.26` остается целевой VLESS identity model: VLESS identity
хранится на уровне клиента, replacement ingress наследует уже выданный client
UUID во время provisioning, а миграция backfill-ит registry из существующих
Xray/VLESS service accesses. Предыдущий stabilization baseline также сохраняет
strict SSH host-key bootstrap scanning, защиту формы от фонового auto-refresh,
фильтрацию runtime reports для deleted instances и очистку orphan runtime state
rows. Основной фокус:
