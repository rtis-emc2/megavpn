...
- Remote agent transport по умолчанию требует HTTPS и поддерживает private CA
  и optional client certificates.
...
- authenticated SMTP требует TLS, а публичные ссылки в письмах требуют HTTPS;
  HTTP разрешен только для loopback development endpoints.
