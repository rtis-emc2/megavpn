...
`7.1.0.4` расширяет runtime учет трафика с Xray/VLESS на managed WireGuard и
OpenVPN, сохраняя режим release stabilization. Основной фокус:
...
- foundation учета трафика: signed agent ingest API, PostgreSQL aggregate
  ...
  страница и managed Xray/WireGuard/OpenVPN byte-counter collectors, которые
  еще требуют live-node validation перед широким rollout;
