...
The Control Plane stores group policy and membership centrally in the generic
access-group registry. Migration `000027_remove_legacy_vless_registry` removes
the superseded VLESS template/membership registries after consolidation. When
...
groups into Xray routing rules scoped by managed client identity.
...
Instance pages expose runtime materialization and apply state only. Membership
changes are available exclusively through `Clients -> Groups`; this prevents a
runtime instance from becoming a second desired-state owner.
...
- Advanced route rules must be a bounded JSON array of supported Xray field
  rules. Unknown fields, nested values and per-client selectors are rejected.
