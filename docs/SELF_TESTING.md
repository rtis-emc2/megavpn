...
- React frontend static guards, production bundle installation and SPA serving smoke
...
- `postgres-migrations-and-integration`: `scripts/ci/postgres-migration-drill.sh`
  ...
  token-storage columns, canonical VLESS access groups and firewall seed groups, then
...
- `systemd-verify` requires `systemd-analyze` and explicit
  `MEGAVPN_SELF_TEST_RUN_SYSTEMD_VERIFY=1`.
- `nginx-t` requires `nginx` and explicit
  `MEGAVPN_SELF_TEST_RUN_NGINX_VERIFY=1`.

To validate a specific nginx config:

```bash
MEGAVPN_SELF_TEST_RUN_NGINX_VERIFY=1 \
MEGAVPN_SELF_TEST_NGINX_CONFIG=/etc/nginx/nginx.conf \
scripts/ci/self-test.sh
```
