...
Control Plane хранит group policy и membership централизованно в общем реестре
access groups. Миграция `000027_remove_legacy_vless_registry` после
консолидации удаляет устаревшие VLESS template/membership registries. Когда
создается или обновляется revision Xray/VLESS instance, активный group catalog
встраивается в rendered instance spec. При apply driver превращает groups в
Xray routing rules, ограниченные управляемой client identity.
...
Instance pages показывают только runtime materialization и apply state.
Membership изменяется исключительно через `Clients -> Groups`, чтобы runtime
instance не становился вторым владельцем desired state.
...
- Advanced route rules должны быть ограниченным по размеру JSON array из
  поддерживаемых Xray field rules. Неизвестные поля, вложенные значения и
  per-client selectors отклоняются.
