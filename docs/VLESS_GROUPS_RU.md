...
- Reprovisioning и rotation Xray UUID сохраняют group текущей client binding.
  Пустой ввод group больше не превращается в synthetic `route`; stale implicit
  metadata fallback-ится в active catalog/default group, а явно выбранная
  неверная group остается validation error.
