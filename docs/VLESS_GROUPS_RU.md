...
1. Откройте конкретный Xray/VLESS instance в `Instances -> Manage`.
2. Перейдите в секцию `VLESS group members`.
3. Выберите target group.
4. Найдите клиентов поиском или вставьте usernames, emails либо client IDs.
5. Нажмите `Add to group`.

Control Plane создает или обновляет существующие `service_accesses` для
выбранного instance и group. Это единственный источник membership; отдельная
таблица membership не создается. Повторное добавление клиента в ту же group
идемпотентно, move между группами сохраняет VLESS UUID, а весь bulk update
ставит максимум один `instance.apply` для affected Xray config.
...
- Instance-level `VLESS group members` использует тот же client access binding
  и не создает duplicate rows. Уникальная VLESS identity клиента сохраняется в
  `client_service_identities` и переиспользуется при смене ingress node или
  переносе клиента между groups.
