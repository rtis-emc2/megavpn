...
The recent-sample table is preview-filtered with the selected values so the
operator can sanity-check the audit handoff context. CSV export is still
generated by the server from the full retained dataset, not only from the
currently loaded recent rows.
