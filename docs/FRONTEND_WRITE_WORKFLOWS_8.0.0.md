...
| Agent registration/onboarding | legacy-only / dedicated workflows | Browser node create creates the control-plane node record only. Secure SSH access-method creation is connected separately, but agent registration/onboarding and live bootstrap validation remain outside this workflow. |
