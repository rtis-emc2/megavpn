...
| Agent registration/onboarding | status, enrollment-token issue/reissue and guided bootstrap job submission connected / registration waiting and inventory sync pending | The new Onboarding tab derives a typed status model from existing operator read APIs: node detail, diagnostics, enrollment-token metadata, bootstrap runs, heartbeat, inventory and access-method metadata. It can issue or reissue an enrollment token through the existing operator `POST /api/v1/nodes/{id}/enrollment-token` and `/rotate` endpoints when the typed model recommends it and `node.bootstrap` is present. It now derives conservative `ssh_bootstrap` and `manual_bundle` availability, requires explicit operator mode selection when needed, confirms the selected mode, submits only `{ bootstrap_mode }` through the existing operator `POST /api/v1/nodes/{id}/bootstrap` endpoint, tracks the returned job/run and keeps the operator on Onboarding. It does not call `/agent/*`, does not set `reinstall_agent` or `force_reenroll` in guided onboarding, does not revoke tokens, does not reveal/download bundles and does not queue inventory sync from Onboarding. Manual bundle reveal/download remains in the Bootstrap tab. Job acceptance is not registration, heartbeat or inventory proof. Guided registration/heartbeat waiting and inventory sync remain Step 4C.2B2, final acceptance/debt closure remains Step 4D and live external-node smoke remains release-validation debt. |
