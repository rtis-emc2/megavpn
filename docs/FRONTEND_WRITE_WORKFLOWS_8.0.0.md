...
| Agent registration/onboarding | status plus enrollment-token issue/reissue connected / bootstrap actions pending | The new Onboarding tab derives a typed status model from existing operator read APIs: node detail, diagnostics, enrollment-token metadata, bootstrap runs, heartbeat and inventory. It can issue or reissue an enrollment token through the existing operator `POST /api/v1/nodes/{id}/enrollment-token` and `/rotate` endpoints when the typed model recommends it and `node.bootstrap` is present. The token is shown only in a transient one-time panel. It does not call `/agent/*`, revoke tokens, start bootstrap, reveal/download bundles or queue inventory sync from Onboarding. Guided bootstrap/inventory actions remain Step 4C.2B, final acceptance/debt closure remains Step 4D and live external-node smoke remains release-validation debt. |
