...
| Agent registration/onboarding | status, enrollment-token issue/reissue, guided bootstrap, registration waiting, heartbeat waiting and guided inventory-sync submission connected; accepted for PR-review evidence in Step 4D.1 | The new Onboarding tab derives a typed status model from existing operator read APIs: node detail, diagnostics, enrollment-token metadata, bootstrap runs, heartbeat, inventory and access-method metadata. It can issue or reissue an enrollment token through the existing operator `POST /api/v1/nodes/{id}/enrollment-token` and `/rotate` endpoints when the typed model recommends it and `node.bootstrap` is present. It derives conservative `ssh_bootstrap` and `manual_bundle` availability, requires explicit operator mode selection when needed, confirms the selected mode, submits only `{ bootstrap_mode }` through the existing operator `POST /api/v1/nodes/{id}/bootstrap` endpoint, tracks the returned job/run and keeps the operator on Onboarding. After real registration and online heartbeat evidence, guided inventory sync uses the existing operator `POST /api/v1/nodes/{id}/inventory/sync` endpoint through the shared `syncNodeInventory` / `useSyncNodeInventory` path owned by `NodeDrawer`. Inventory sync requires `node.write` separately from `node.bootstrap`, needs explicit acknowledgement, tracks returned jobs locally, does not queue duplicates while an inventory job is active or stalled, and does not treat accepted/queued/running jobs as synchronized inventory. Ready requires registration, heartbeat, inventory and a healthy communication state. It does not call `/agent/*`, does not set `reinstall_agent` or `force_reenroll` in guided onboarding, does not revoke tokens and does not reveal/download bundles. Manual bundle reveal/download remains in the Bootstrap tab. Release debt accounting remains Step 4D.2, and live external-node smoke remains release-validation debt. |
...
| Enrollment tokens | fully connected | `GET /enrollment-tokens`, `POST /enrollment-token`, `POST /enrollment-token/rotate`, `DELETE /enrollment-tokens/{token_id}`. Security and Onboarding issue/reissue use the same one-time consumer path. Plaintext tokens are shown only once from create/rotate responses and cleared on close, target change, permission loss, stale response or unmount. |
...
| Agent Identity Revoke | fully connected | `POST /api/v1/nodes/{id}/agent-identity/revoke` through `revokeNodeAgentIdentity` / `useRevokeNodeAgentIdentity`; requires `node.bootstrap`, exact node-name confirmation and reason. Backend atomically revokes the active identity and active unused enrollment tokens while preserving node/history. Old credentials are rejected. Recovery requires explicit new enrollment-token issuance; no token is issued automatically. |
| Node Reboot | fully connected | `POST /api/v1/nodes/{id}/reboot` through `createNodeRebootJob` / `useCreateNodeRebootJob`; requires `node.bootstrap`, exact confirmation and reason. Success means an asynchronous job was queued. Agent scheduling is not reboot execution or recovery; the operator follows progress through Jobs and the UI makes no optimistic lifecycle transition. |
| Emergency Cleanup | fully connected | `POST /api/v1/nodes/{id}/emergency-cleanup` through `createNodeEmergencyCleanupJob` / `useCreateNodeEmergencyCleanupJob`; requires `node.bootstrap`, maintenance mode, exact `services_only`/`full_node` scope, confirmation, reason and acknowledgements. Browser sends no targets/paths/units/configs. Backend returns a safe plan summary; queue acceptance is not completion evidence. |
| Stale Rotation Preview | fully connected read-only workflow | `GET /api/v1/nodes/{id}/diagnostics/stale-rotation` through `getNodeStaleRotationPreview` / `useNodeStaleRotationPreview`; requires `node.read`. Backend owns stale classification and the complete safe candidate set; the frontend has no stale threshold or partial candidate selection. |
| Stale Rotation Clear | fully connected | `POST /api/v1/nodes/{id}/diagnostics/clear-stale-rotation` through `clearNodeStaleRotation` / `useClearNodeStaleRotation`; requires `node.bootstrap`, exact confirmation, reason, acknowledgement and the exact backend-safe candidate set. Preview changes require refresh and re-review. Active identity/token is preserved; this is pending-state cleanup, not token rotation or issuance. |
...
| Inventory view/sync | fully connected | `GET /inventory` and `POST /inventory/sync`; standalone sync requires confirmation and navigates to Jobs. Guided onboarding sync requires `node.write`, confirmation plus acknowledgement, stays on Onboarding, tracks the returned job ID and waits for backend inventory evidence before completing the milestone. Inventory payload is rendered as text outside onboarding status cards. |
...
Agent Lifecycle Controls integration evidence uses disposable real operator
HTTP/PostgreSQL, authenticated signed-agent polling/result submission,
conflict handling and redaction assertions. Signed-agent execution results are
simulated through the real protocol; no real host reboot, cleanup, systemd,
filesystem, network or self-removal command was executed. Live external-node
validation and Step 4B debt accounting remain open.
