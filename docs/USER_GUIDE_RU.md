...
- В том же `Manage` редактируются `VLESS outbound groups`: это JSON-массив
  групп Xray routing. `Default outbound group` задает группу по умолчанию.
...
4. В `Instances -> Manage` у VLESS instance установите `Egress mode = egress node`
   и выберите нужную egress node.
