...
services по backend-defined reconciliation path.

## 8.2 Управление жизненным циклом агента и узла

Откройте `Nodes`, выберите нужный узел и перейдите на вкладку `Lifecycle`.
Панель показывает состояние идентичности агента, maintenance mode, состояние
канала связи, состояние ротации токена и backend-preview зависшей ротации.
Доступность опасных действий зависит от актуального состояния и разрешений
оператора. Backend остается окончательной точкой авторизации и проверки.

Для read-only preview зависшей ротации требуется `node.read`. Для отзыва
идентичности, постановки перезагрузки в очередь, Emergency Cleanup и очистки
зависшей ротации требуется `node.bootstrap`.

### Отзыв идентичности агента

1. Проверьте текущее состояние идентичности, канала и последних заданий.
2. Откройте диалог `Revoke agent identity`.
3. Прочитайте последствия операции.
4. Введите точное имя выбранного узла.
5. Укажите операционную причину без токенов, ключей и конфигураций.
6. Подтвердите понимание последствий.
7. Выполните отзыв.
8. Убедитесь по данным backend, что состояние идентичности стало `revoked`.
9. Выпускайте новый enrollment token только при осознанном запуске процедуры
   восстановления.

После отзыва прежняя учетная информация агента перестает проходить
аутентификацию. Активные неиспользованные enrollment tokens также отзываются.
Запись узла, inventory, задания и audit/history сохраняются. Операция не
удаляет agent software с host и не запускает автоматическую повторную
регистрацию. Восстановление требует отдельного явного выпуска enrollment token
и контролируемого onboarding.

### Постановка перезагрузки в очередь

1. Проверьте состояние агента и канала связи.
2. Откройте диалог `Reboot node`.
3. Введите точное имя узла.
4. Укажите операционную причину.
5. Подтвердите ожидаемое нарушение доступности.
6. Поставьте задание в очередь.
7. Откройте `Jobs / Activity` и отслеживайте backend evidence.

Различайте три состояния:

1. задание принято и поставлено в очередь;
2. агент сообщил, что команда перезагрузки запланирована;
3. узел действительно восстановился после перезагрузки и снова передает
   актуальные heartbeat/runtime данные.

Первые два состояния не доказывают третье. UI не переводит узел оптимистично в
`offline` или `rebooting` и не заявляет о восстановлении без фактических данных.

### Аварийная очистка

Перед началом maintenance mode должен быть включен отдельным действием. Также
необходимы `node.bootstrap`, активная идентичность агента и работоспособный
канал связи.

Доступные области очистки:

- `services_only`: удаляется только поддерживаемый runtime управляемых сервисов;
  агент сохраняется;
- `full_node`: очищается весь поддерживаемый MegaVPN-managed runtime; запись
  узла, inventory, задания и audit/history в базе сохраняются.

Параметр `include_agent` доступен только для `full_node`. Он запрашивает
self-removal агента после успешной очистки и после подтвержденного control-plane
acknowledgement результата. Новый recovery enrollment token автоматически не
выпускается.

Порядок действий:

1. Отдельно включите maintenance mode.
2. Проверьте состояние узла, агента, канала и активных заданий.
3. Откройте `Emergency Cleanup`.
4. Выберите точный scope: `services_only` или `full_node`.
5. При `full_node` при необходимости отдельно запросите удаление агента.
6. Введите точное имя узла.
7. Укажите операционную причину.
8. Подтвердите обязательные предупреждения; для удаления агента требуется
   дополнительное подтверждение.
9. Поставьте задание в очередь.
10. Проверьте `Jobs / Activity` и фактическое состояние узла на staging/live
    стенде.

Ответ о постановке в очередь не является доказательством исполнения. Shared
Nginx намеренно не останавливается и не отключается. Browser не отправляет
произвольные paths, systemd units, targets, команды или конфигурации: безопасный
план строит backend. Emergency Cleanup не выполняет автоматически retire,
reboot или revoke identity.

### Очистка зависшего состояния ротации

1. Просмотрите backend-preview зависшей ротации.
2. Проверьте безопасные и исключенные backend candidates.
3. Обновите preview непосредственно перед операцией.
4. Откройте диалог очистки.
5. Введите точное имя узла.
6. Укажите операционную причину.
7. Подтвердите отмену полного backend-safe набора.
8. Очистите точный набор, полученный из preview.
9. Проверьте результат и обновленные diagnostics.

Операция отменяет только stale rotation jobs, которые backend признал
безопасными, и очищает только связанную pending rotation state. Активная
идентичность и активный токен сохраняются. Новый токен не выпускается и ротация
не выполняется. Если backend-safe набор изменился после preview, UI требует
обновить данные и повторно проверить последствия; частичный выбор не
поддерживается.

> **Предупреждение: disposable protocol evidence не является live-node
> evidence.** Перед production use controlled staging/live smoke должен
> подтвердить реальную перезагрузку и восстановление, контролируемую Emergency
> Cleanup, фактическое влияние на сервисы, rollback/re-enrollment procedure и
> восстановление операторского доступа.
