...
7. `Instances -> Create from pack` показывает каталог service packs. Default
   ...
   API.
8. `Clients -> Groups` показывает VLESS client access groups. `Instances ->
   VLESS groups` остается compatibility catalog для default route, current node
   exit, ad-blocked default и blocked access.
...
- `Instances -> VLESS groups`: compatibility catalog для просмотра policy
  templates. Членство клиентов там больше не редактируется; используйте
  `Clients -> Groups`.
