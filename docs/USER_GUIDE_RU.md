...
8. Если pack содержит VLESS, настройте `VLESS routing`:
...
- В том же `Manage` редактируются `VLESS outbound groups`. Это политики
  доступа на этапе provisioning, а не просто названия:
  ...
  `Default outbound group` задает группу, которая используется, если client
  binding не указал конкретную группу.
...
- `Clients -> Access -> Add route`: для отдельных route-policy правил можно
  ...
  route policy выполните `Nodes -> Manage -> Sync route policy` на ingress node.
...
5. Нажмите `Apply` у instance.
6. Если используются client route rules, выполните `Sync route policy`.
