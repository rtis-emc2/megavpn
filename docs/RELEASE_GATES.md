...
For `7.0.1.11`, these gates are used as release promotion evidence. A release can be published with documented product gaps, but not with unknown install, migration, agent-channel, node-cleanup or runtime-apply behavior.
...
- Traffic camouflage profiles validate Nginx and Xray config before apply,
  require an explicit HTTP(S) fallback website in API/smoke flows, and expose
  rollback on failed `nginx -t` or runtime validation.
