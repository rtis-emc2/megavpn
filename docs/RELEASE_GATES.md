...
For `8.0.0-pre.2`, these gates are used as release promotion evidence. A release can be published with documented product gaps, but not with unknown install, migration, agent-channel, node-cleanup or runtime-apply behavior.
...
- `npm ci`, `npm run audit:ci`, frontend typecheck, lint, tests, i18n parity and
  the reproducible production build pass. The audit guard rejects every
  unreviewed high/critical advisory. The temporary React Router RSC advisory
  exception is accepted only for the exact reviewed version while static guards
  prove that the console remains a declarative `BrowserRouter` SPA without RSC,
  server-router or data-router APIs.
...
- Operational JavaScript passes `node --check`; frontend typecheck and build
  validate React source and generated modules. `scripts/ci/frontend-static-guards.sh`
  and `scripts/ci/frontend-serving-smoke.sh` pass for the production bundle.
...
token-storage columns, canonical VLESS access groups and firewall seed groups, then the
