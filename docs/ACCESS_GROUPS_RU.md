...
Frontend `8.0.0` подключает VLESS workflow в `Clients -> Groups` напрямую к
generic client access group backend: create/edit, member preview/apply, member
removal, scope update и sync preview/apply не требуют `/legacy/`. Non-VLESS
