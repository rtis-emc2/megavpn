...
Примечание для frontend `8.0.0` RC1: новая React/Vite-консоль показывает
`Clients -> Groups` как primary location, но member preview/apply workflows
остаются disabled/legacy-only до полного подключения backend preview/apply.
Для продуктивных изменений membership в RC1 используйте `/legacy/`.
