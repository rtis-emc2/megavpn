...
- `client_access_groups` и `client_access_group_memberships` являются
  единственным desired-state источником членства для актуального кода.
- VLESS-ключи в `service_accesses.metadata_json` являются материализованной
  runtime-проекцией. Их можно использовать для проверки convergence на node,
  но нельзя читать как желаемую группу клиента.
- Миграция `000027_remove_legacy_vless_registry` консолидирует и удаляет
  `vless_group_templates`, `vless_group_memberships` и
  `client_access_group_migration_conflicts`. Их API отсутствует в актуальном
  коде. Перед удалением устаревших реестров миграция сохраняет ограниченное по
  размеру свидетельство retirement в `audit_events`.

`client_access_groups.service_code` обозначает логический протокол доступа,
например `vless` или `l2tp`. Это не runtime driver code (`xray-core`, `xl2tpd`,
`ipsec`, `nginx`), поэтому прямой FK на `service_definitions.code` смешал бы две
разные сущности.
...
- VLESS policy JSON ограничен по размеру, отклоняет неизвестные поля и
  принимает только проверенные routing modes и rule fields. Per-client
  selectors нельзя подмешать через advanced policy JSON.
