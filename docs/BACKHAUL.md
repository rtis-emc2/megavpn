...
| `openvpn_udp` | L3 | Writes config/static key, writes systemd unit, enables unit | UDP fallback when WireGuard is unsuitable. |
...
- Backhaul jobs are agent-only; the worker refuses to execute `node.backhaul.apply`.
...
7. Create client access route with remote egress node.
8. Queue route policy sync for the ingress node.
9. Verify route projection uses `managed_backhaul` and route policy job reports `enforced=true`.
...
- Add deep health probes for throughput, MTU/MSS and packet-loss telemetry.
- Add rollback/removal command for disabled route policies and retired backhaul links.
