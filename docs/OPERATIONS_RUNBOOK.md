...
MEGAVPN_DATABASE_DSN='postgres://...' \
  scripts/ops/database-integrity-audit.sh
```

The command fails when an instance references a missing or foreign revision,
an audit actor references a missing platform user, or an access-group
membership disagrees with its owning group's logical service. Legacy VLESS
membership rows and migration-conflict records are reported as warnings because
they require an explicit retention decision rather than automatic deletion.
After repairing every critical row, validate constraints that were installed as
`NOT VALID` during a compatibility rollout:

```bash
MEGAVPN_DATABASE_DSN='postgres://...' \
  scripts/ops/database-integrity-audit.sh --validate-constraints
```
...
git add -A
git commit -m "Release 8.0.0-pre.1 clean import"
git tag -f v8.0.0-pre.1
git push --force-with-lease origin release-clean:main
git push --force-with-lease origin v8.0.0-pre.1
```
