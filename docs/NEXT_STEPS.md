...
Текущая точка фиксации: `0.6.10.1-alpha`.
Следующая итерация: `0.6.10.1-alpha`.
Канонический репозиторий: `github.com/rtis-emc2/megavpn`.

1. Зафиксировать CI green после managed backhaul kernel enforcement: `bash -n`, `go test`, `go vet`, `go build`, `scripts/build.sh`, `git diff --check`.
