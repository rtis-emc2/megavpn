...
Текущая точка фиксации: `0.6.10.6-alpha`.
Следующая итерация: `0.6.10.7-alpha`.
Канонический репозиторий: `github.com/rtis-emc2/megavpn`.

1. На реальных nodes проверить agent version drift UX: версия агента в Nodes, per-node Update, bulk Update all agents, успешное обновление через SSH bootstrap reinstall и исчезновение update action после heartbeat новой версии.
