...
- L2TP/IPsec deployment дополнительно имеет установленную IPsec security
  association и IPv4-адрес на managed PPP interface;
...
нетронутым.
Apply и Cleanup также ждут удаления deployment-specific PPP interface перед
заменой или удалением runtime. Новый Apply не переиспользует stale interface
от предыдущего запуска. Startup ждет появления PPP interface и его IPv4-адреса
до установки provider default route. Финальный L2TP/IPsec probe требует два
последовательных полных наблюдения и отдельно показывает отсутствующий слой:
systemd unit, IPsec association, PPP interface, адрес, route либо policy rule.
...
| Runtime probe показывает отсутствующие L2TP слои | Apply возвращает конкретный отсутствующий unit, IPsec SA, PPP address, provider route либо policy rule | Выполнить diagnostic commands из job evidence, исправить первый отсутствующий слой и повторить Apply |
