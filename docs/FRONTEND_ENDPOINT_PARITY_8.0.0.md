...
| `GET /api/v1/nodes/{id}`, `GET /api/v1/nodes/{id}/diagnostics`, `GET /api/v1/nodes/{id}/enrollment-tokens`, `GET /api/v1/nodes/{id}/bootstrap-runs`, `GET /api/v1/nodes/{id}/inventory` | guided agent onboarding status | existing node detail/diagnostics/enrollment/bootstrap/inventory hooks plus `nodeOnboarding` derivation | Nodes/Onboarding | read-only connected | Guided read-only status UI derives milestones from existing operator read APIs only. It does not call `/agent/*`, does not execute enrollment/bootstrap/inventory mutations, does not render token plaintext, token hashes, secret refs or bootstrap bundle payloads, and polls only safe read queries while the onboarding tab is active and not terminal. Guided onboarding mutations remain pending Step 4C.2; final acceptance/debt closure remains pending Step 4D; live external-node smoke remains open. |
