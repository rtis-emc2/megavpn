...
| `GET /api/v1/nodes/{id}`, `GET /api/v1/nodes/{id}/diagnostics`, `GET /api/v1/nodes/{id}/enrollment-tokens`, `GET /api/v1/nodes/{id}/bootstrap-runs`, `GET /api/v1/nodes/{id}/inventory`, `GET /api/v1/nodes/{id}/access-methods`, `POST /api/v1/nodes/{id}/enrollment-token`, `POST /api/v1/nodes/{id}/enrollment-token/rotate`, `POST /api/v1/nodes/{id}/bootstrap`, `POST /api/v1/nodes/{id}/inventory/sync` | guided agent onboarding status, enrollment-token issue/reissue, guided bootstrap submission and guided inventory-sync submission | existing node detail/diagnostics/enrollment/bootstrap/inventory/access-method hooks plus `nodeOnboarding` and `nodeBootstrapReadiness` derivation; `createEnrollmentToken`; `rotateEnrollmentToken`; `bootstrapNode`; `syncNodeInventory`; matching hooks | Nodes/Onboarding | connected for status, enrollment-token issue/reissue, guided bootstrap mode selection/job submission, registration waiting, heartbeat waiting and inventory-sync submission; accepted for PR-review evidence in Step 4D.1 | Guided status UI derives milestones from existing operator read APIs. Onboarding can issue or reissue an enrollment token only when the typed model recommends it and the operator has `node.bootstrap`. It derives safe availability for `ssh_bootstrap` and `manual_bundle`, requires explicit operator selection when needed, submits through the existing operator `POST /api/v1/nodes/{id}/bootstrap` endpoint, and sends only `{ bootstrap_mode }` for the guided initial request. After real registration and online heartbeat evidence, guided inventory sync uses only the existing operator `POST /api/v1/nodes/{id}/inventory/sync` endpoint through `syncNodeInventory` / `useSyncNodeInventory`, owned by `NodeDrawer`, and requires `node.write` separately from `node.bootstrap`. Accepted inventory jobs are tracked as asynchronous jobs and are not treated as synchronized inventory. Ready requires registration, heartbeat, inventory evidence and a healthy communication state. Guided onboarding does not set `reinstall_agent` or `force_reenroll`, does not call `/agent/*`, does not revoke tokens and does not reveal/download manual bundles. Plaintext token values are consumed only from the immediate create/rotate response through the one-time panel path and are not retained in query data, mutation data, storage, URLs, logs or toasts. Job acceptance is not treated as bootstrap success, registration, heartbeat or inventory evidence. Release debt accounting remains Step 4D.2; live external-node smoke remains open. |
| `GET /api/v1/nodes/{id}/diagnostics/stale-rotation` | read-only stale-rotation preview | `getNodeStaleRotationPreview`; `useNodeStaleRotationPreview`; `NodeLifecycleControlsPanel` | Nodes/Lifecycle | connected / read-only operator evidence | Requires `node.read`. Candidate safety and reasons come from the backend; no secret or job payload is rendered and the frontend has no stale timeout. The hook does not poll automatically (`refetchInterval: false`); the panel supports explicit manual refresh and refresh-before-clear review. |
...
| `POST /api/v1/nodes/{id}/agent-identity/revoke` | Agent Identity Revoke | `revokeNodeAgentIdentity`; `useRevokeNodeAgentIdentity`; `NodeAgentIdentityRevokeDialog` | Nodes/Lifecycle | connected | Requires `node.bootstrap`, exact node-name confirmation and reason. Backend revokes the active identity and active enrollment credentials atomically. The browser performs no optimistic identity change and issues no replacement token; node/history are preserved and recovery requires explicit enrollment-token issuance. |
| `POST /api/v1/nodes/{id}/reboot` | Node Reboot | `createNodeRebootJob`; `useCreateNodeRebootJob`; `NodeRebootDialog` | Nodes/Lifecycle | connected | Requires `node.bootstrap`, exact node-name confirmation and reason. Response is a redacted `Job`; success means queued only. An agent scheduling result is not reboot or recovery evidence, and the browser does not set an optimistic offline/rebooting state. |
| `POST /api/v1/nodes/{id}/emergency-cleanup` | Emergency Cleanup | `createNodeEmergencyCleanupJob`; `useCreateNodeEmergencyCleanupJob`; `NodeEmergencyCleanupDialog` | Nodes/Lifecycle | connected | Requires `node.bootstrap`, maintenance mode, exact `services_only`/`full_node` scope, exact confirmation, reason and acknowledgements. The browser submits no targets, paths, units or configs. Backend returns a safe plan summary; HTTP 202 means queued only. |
| `POST /api/v1/nodes/{id}/diagnostics/clear-stale-rotation` | Stale Rotation Clear | `clearNodeStaleRotation`; `useClearNodeStaleRotation`; `NodeStaleRotationClearDialog` | Nodes/Lifecycle | connected | Requires `node.bootstrap`. `expected_job_ids` come only from the complete backend-safe preview set. Changed-set conflict requires refresh/re-review; response validation requires active identity preservation. The operation clears correlated pending state and does not rotate or issue a token. |
| `GET /api/v1/nodes/{id}/inventory`, `POST /api/v1/nodes/{id}/inventory/sync` | inventory view/sync | `getNodeInventory`; `syncNodeInventory`; `useNodeInventory`; `useSyncNodeInventory` | Nodes/Inventory and Nodes/Onboarding | connected | Standalone Inventory sync keeps the existing Jobs navigation. Guided Onboarding sync uses the same wrapper/hook from `NodeDrawer`, requires `node.write`, stays on Onboarding, records the returned job ID transiently and does not mark inventory synchronized until backend inventory evidence appears. Inventory payload is rendered as text outside onboarding status cards. |
