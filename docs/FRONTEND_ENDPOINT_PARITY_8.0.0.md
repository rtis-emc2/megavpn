...
| Backend endpoint family | Legacy usage | New wrapper / hook | UI page | Status | Invalidation / security notes |
| --- | --- | --- | --- | --- | --- |
| `GET/POST /api/v1/nodes`, `GET/PUT/DELETE /api/v1/nodes/{id}` | node CRUD | `listNodes`; `getNode`; `retireNode`; `useNodes`; `useNodeDetail`; `useRetireNode`; create/update missing from new UI | Nodes | connected read + retire / legacy-only create/update | Existing node list/detail and normal retire are connected. Create/edit stay legacy-only. Retire requires confirmation and backend validation. |
...
| `GET/PUT /api/v1/nodes/{id}/access-methods`, `POST /ssh/host-key-scan`, `/ssh/sessions`, `GET /ssh/terminal` | bootstrap/terminal | `listNodeAccessMethods`; `acceptNodeHostKey`; `scanNodeHostKey`; `launchNodeSshSession`; matching hooks | Nodes/Security + Terminal | connected for existing SSH methods | Host-key scan/pin and SSH session ticket launch are connected. SSH credentials stay server-side; terminal URL is shown only as a short-lived one-time backend URL. Creating SSH access methods with new secret material remains disabled/not exposed. |
| `POST /api/v1/nodes/{id}/bootstrap`, `GET /bootstrap-runs`, `GET /bootstrap-runs/{run_id}/bundle` | bootstrap | `bootstrapNode`; `reinstallOrUpdateNodeAgent`; `listNodeBootstrapRuns`; matching hooks | Nodes/Bootstrap | connected except bundle reveal | SSH bootstrap/manual bundle job queueing and reinstall are connected with confirmation and job tracking. Manual bundle secret reveal is intentionally not exposed in the new UI. |
| `POST /api/v1/nodes/{id}/agent-token/rotate`, `/enrollment-token`, `/enrollment-token/rotate`, `GET /enrollment-tokens`, `DELETE /enrollment-tokens/{token_id}` | token control | `rotateNodeAgentToken`; `listEnrollmentTokens`; `createEnrollmentToken`; `rotateEnrollmentToken`; `revokeEnrollmentToken`; matching hooks | Nodes/Security | connected | Enrollment token plaintext is shown only from create/rotate mutation result and cleared on close. List/revoke show metadata/hints only. Agent token rotation returns a redacted signed job. |
...
| `DELETE /api/v1/clients/{id}/accesses/{access_id}` | service access delete | `deleteClientAccess`; `useDeleteClientAccess` | Clients detail / Maintenance | connected | Confirmation required; result counts and queued job counts are shown. Per-access revoke remains backend-missing. |
| `POST /api/v1/clients/{id}/accesses/{access_id}/revoke` | per-access revoke | `revokeClientAccess` disabled wrapper | Clients detail / Maintenance | backend-missing | UI disables this exact sub-action with reason. Use client-level revoke or service-access delete where appropriate. |
| `GET/POST/DELETE /api/v1/clients/{id}/routes` | client routes | `listClientRoutes`; `createClientRoute`; `deleteClientRoute`; matching hooks | Clients detail / Routes | connected | List/create/delete are wired with backend validation and destructive confirmation. Route update remains backend-missing because no `PUT/PATCH /routes/{id}` endpoint exists. |
