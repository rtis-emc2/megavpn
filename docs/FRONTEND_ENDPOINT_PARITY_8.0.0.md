...
| `POST /api/v1/nodes/{id}/bootstrap`, `GET /bootstrap-runs`, `POST /bootstrap-runs/{run_id}/bundle/reveal`, `POST /bootstrap-runs/{run_id}/bundle/download`, compatibility `GET /bootstrap-runs/{run_id}/bundle` | bootstrap | `bootstrapNode`; `reinstallOrUpdateNodeAgent`; `listNodeBootstrapRuns`; matching hooks | Nodes/Bootstrap | connected except bundle reveal UI | SSH bootstrap/manual bundle job queueing and reinstall are connected with confirmation and job tracking. Manual bundle reveal/download APIs are backend-hardened with `node.bootstrap`, CSRF for cookie sessions, scoped run lookup, no-store headers and fail-closed audit, but the new UI reveal/download workflow remains pending. The retained GET endpoint is compatibility/deprecated; new clients must use POST reveal/download. |
