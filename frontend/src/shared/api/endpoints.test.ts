...
describe('external egress endpoints', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (method === 'GET' && url.pathname === '/api/v1/external-egress/catalog') {
        return json([{ code: 'l2tp_ipsec', label: 'L2TP over IPsec', runtime_support: 'ready', import_formats: ['json'], credential_modes: ['psk'] }]);
      }
      if (method === 'GET' && url.pathname === '/api/v1/external-egress/profiles') {
        return json([{ id: 'profile-1', display_name: 'Provider', protocol: 'l2tp_ipsec', status: 'active', deployments: [] }]);
      }
      if (method === 'POST' && url.pathname === '/api/v1/external-egress/import:preview') {
        return json({ protocol: 'l2tp_ipsec', transport: 'udp_ipsec', endpoint_host: 'vpn.example.test', endpoint_port: 1701, runtime_support: 'ready', import_format: 'json' });
      }
      if (method === 'POST' && url.pathname === '/api/v1/external-egress/profiles') {
        return json({ id: 'profile-1', ...body }, 201);
      }
      if (method === 'POST' && url.pathname === '/api/v1/external-egress/profiles/profile-1/deployments') {
        return json({ id: 'deployment-1', profile_id: 'profile-1', node_id: String(body?.node_id || ''), status: 'pending' }, 201);
      }
      ...
      if (method === 'DELETE' && url.pathname === '/api/v1/external-egress/deployments/deployment-1') {
        return json({ id: 'deployment-1', profile_id: 'profile-1', node_id: 'node-1', status: 'deleted' });
      }
  ...
  it('uses the backend contract for catalog, preview, profile, deployment, apply and removal', async () => {
    expect(await listExternalEgressCatalog()).toHaveLength(1);
    expect(await listExternalEgressProfiles()).toHaveLength(1);
    await previewExternalEgressImport({ protocol: 'l2tp_ipsec', format: 'json', content: '{"server":"vpn.example.test"}' });
    await createExternalEgressProfile({
      display_name: 'Provider',
      protocol: 'l2tp_ipsec',
      status: 'active',
      secrets: { username: 'operator', password: 'secret', preshared_key: 'psk' },
    });
    await createExternalEgressDeployment('profile-1', { node_id: 'node-1', routing_table: 'auto', route_metric: 100 });
    await applyExternalEgressDeployment('deployment-1');
    await deleteExternalEgressDeployment('deployment-1');

    expect(calls.map((call) => `${call.method} ${call.path}`)).toEqual([
      'GET /api/v1/external-egress/catalog',
      'GET /api/v1/external-egress/profiles',
      'POST /api/v1/external-egress/import:preview',
      'POST /api/v1/external-egress/profiles',
      'POST /api/v1/external-egress/profiles/profile-1/deployments',
      'POST /api/v1/external-egress/deployments/deployment-1/apply',
      'DELETE /api/v1/external-egress/deployments/deployment-1',
    ]);
    expect(calls.filter((call) => call.method !== 'GET').every((call) => call.headers['x-megavpn-csrf'] === '1')).toBe(true);
    expect(calls[3].body).not.toHaveProperty('profile_key');
  });
});
