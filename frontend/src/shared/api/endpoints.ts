...
export function nodeSshTerminalPath(nodeId: string): string {
  return `/api/v1/nodes/${encodeURIComponent(nodeId)}/ssh/terminal`;
}
  ...
  return sendJSON<Omit<SshSessionLaunchResult, 'terminal_url'>>(`/api/v1/nodes/${encodeURIComponent(nodeId)}/ssh/sessions`, 'POST', {})
    .then((result) => {
      ...
      const terminalURL = session
        ? apiURL(`${nodeSshTerminalPath(nodeId)}${queryString({ session })}`)
        : '';
...
export function listExternalEgressCatalog(): Promise<ExternalEgressProtocol[]> {
  return apiRequest<ExternalEgressProtocol[]>('/api/v1/external-egress/catalog');
}

export function previewExternalEgressImport(input: ExternalEgressImportPreviewRequest): Promise<ExternalEgressImportPreview> {
  return sendJSON<ExternalEgressImportPreview>('/api/v1/external-egress/import:preview', 'POST', input);
}
...
export function getExternalEgressProfile(profileId: string): Promise<ExternalEgressProfile> {
  return apiRequest<ExternalEgressProfile>(`/api/v1/external-egress/profiles/${encodeURIComponent(profileId)}`);
}

export function createExternalEgressProfile(input: ExternalEgressProfileInput): Promise<ExternalEgressProfile> {
  return sendJSON<ExternalEgressProfile>('/api/v1/external-egress/profiles', 'POST', input);
}

export function updateExternalEgressProfile(profileId: string, input: ExternalEgressProfileInput): Promise<ExternalEgressProfile> {
  return sendJSON<ExternalEgressProfile>(`/api/v1/external-egress/profiles/${encodeURIComponent(profileId)}`, 'PATCH', input);
}

export function deleteExternalEgressProfile(profileId: string): Promise<ExternalEgressProfile> {
  return apiRequest<ExternalEgressProfile>(`/api/v1/external-egress/profiles/${encodeURIComponent(profileId)}`, { method: 'DELETE' });
}

export function createExternalEgressDeployment(profileId: string, input: ExternalEgressDeploymentInput): Promise<ExternalEgressDeployment> {
  return sendJSON<ExternalEgressDeployment>(`/api/v1/external-egress/profiles/${encodeURIComponent(profileId)}/deployments`, 'POST', input);
}

export function deleteExternalEgressDeployment(deploymentId: string): Promise<ExternalEgressDeployment> {
  return apiRequest<ExternalEgressDeployment>(`/api/v1/external-egress/deployments/${encodeURIComponent(deploymentId)}`, { method: 'DELETE' });
}

export function applyExternalEgressDeployment(deploymentId: string): Promise<Job> {
  return sendJSON<Job>(`/api/v1/external-egress/deployments/${encodeURIComponent(deploymentId)}/apply`, 'POST', {});
}

export function probeExternalEgressDeployment(deploymentId: string): Promise<Job> {
  return sendJSON<Job>(`/api/v1/external-egress/deployments/${encodeURIComponent(deploymentId)}/probe`, 'POST', {});
}

export function cleanupExternalEgressDeployment(deploymentId: string): Promise<Job> {
  return sendJSON<Job>(`/api/v1/external-egress/deployments/${encodeURIComponent(deploymentId)}/cleanup`, 'POST', {});
}
...
export const endpoints = {
  ...
  audit: () => apiRequest<AuditEvent[]>('/api/v1/audit?limit=200'),
  runtimePreflight: () => apiRequest<RuntimePreflight>('/api/v1/runtime/preflight'),
