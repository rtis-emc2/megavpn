...
import {
  ...
  createExternalEgressDeployment,
  createExternalEgressProfile,
  createServicePack,
...
export function useExternalEgressCatalog(options?: QueryOptions<ExternalEgressProtocol[]>) {
  return useQuery({
    queryKey: ['external-egress-catalog'],
    queryFn: endpoints.externalEgressCatalog,
    staleTime: stale.slow,
    ...options,
  });
}

export function useExternalEgressProfiles(options?: QueryOptions<ExternalEgressProfile[]>) {
  return useQuery({
    queryKey: ['external-egress-profiles'],
    queryFn: listExternalEgressProfiles,
    staleTime: stale.normal,
    ...options,
  });
}
...
export function useExternalEgressProfile(profileId: string | undefined, options?: QueryOptions<ExternalEgressProfile>) {
  return useQuery({
    ...
    queryFn: () => getExternalEgressProfile(profileId || ''),
...
function invalidateExternalEgressQueries(
  queryClient: ReturnType<typeof useQueryClient>,
  profileId?: string,
  job?: Job,
) {
  void queryClient.invalidateQueries({ queryKey: ['external-egress-profiles'] });
  void queryClient.invalidateQueries({ queryKey: ['client-access-groups'] });
  void queryClient.invalidateQueries({ queryKey: ['nodes'] });
  void queryClient.invalidateQueries({ queryKey: ['jobs'] });
  if (profileId) void queryClient.invalidateQueries({ queryKey: ['external-egress-profile', profileId] });
  if (job?.id) void queryClient.invalidateQueries({ queryKey: ['job', job.id] });
}

export function usePreviewExternalEgressImport() {
  return useMutation<ExternalEgressImportPreview, Error, ExternalEgressImportPreviewRequest>({
    mutationFn: previewExternalEgressImport,
  });
}

export function useCreateExternalEgressProfile() {
  const queryClient = useQueryClient();
  return useMutation<ExternalEgressProfile, Error, ExternalEgressProfileInput>({
    mutationFn: createExternalEgressProfile,
    onSuccess: (profile) => invalidateExternalEgressQueries(queryClient, profile.id),
  });
}

export function useUpdateExternalEgressProfile() {
  const queryClient = useQueryClient();
  return useMutation<ExternalEgressProfile, Error, { profileId: string; input: ExternalEgressProfileInput }>({
    mutationFn: ({ profileId, input }) => updateExternalEgressProfile(profileId, input),
    onSuccess: (profile) => invalidateExternalEgressQueries(queryClient, profile.id),
  });
}

export function useDeleteExternalEgressProfile() {
  const queryClient = useQueryClient();
  return useMutation<ExternalEgressProfile, Error, string>({
    mutationFn: deleteExternalEgressProfile,
    onSuccess: (profile) => invalidateExternalEgressQueries(queryClient, profile.id),
  });
}

export function useCreateExternalEgressDeployment() {
  const queryClient = useQueryClient();
  return useMutation<ExternalEgressDeployment, Error, { profileId: string; input: ExternalEgressDeploymentInput }>({
    mutationFn: ({ profileId, input }) => createExternalEgressDeployment(profileId, input),
    onSuccess: (deployment) => invalidateExternalEgressQueries(queryClient, deployment.profile_id),
  });
}

export function useDeleteExternalEgressDeployment() {
  const queryClient = useQueryClient();
  return useMutation<ExternalEgressDeployment, Error, string>({
    mutationFn: deleteExternalEgressDeployment,
    onSuccess: (deployment) => invalidateExternalEgressQueries(queryClient, deployment.profile_id),
  });
}

function useExternalEgressDeploymentJob(
  action: (deploymentId: string) => Promise<Job>,
) {
  const queryClient = useQueryClient();
  return useMutation<Job, Error, { deploymentId: string; profileId: string }>({
    mutationFn: ({ deploymentId }) => action(deploymentId),
    onSuccess: (job, input) => invalidateExternalEgressQueries(queryClient, input.profileId, job),
  });
}

export function useApplyExternalEgressDeployment() {
  return useExternalEgressDeploymentJob(applyExternalEgressDeployment);
}

export function useProbeExternalEgressDeployment() {
  return useExternalEgressDeploymentJob(probeExternalEgressDeployment);
}

export function useCleanupExternalEgressDeployment() {
  return useExternalEgressDeploymentJob(cleanupExternalEgressDeployment);
}
