...
describe('ClientGroupsPage', () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (method === 'GET' && url.pathname === '/api/v1/auth/me') {
        return json({
          ...
          roles: ['operator'],
          permissions: ['node.read', 'access_group.read', 'access_group.policy.write'],
        });
      }
  ...
  it('assigns a global client group to an external egress profile without instance-local routing fields', async () => {
    const user = userEvent.setup();
    renderPage();
    ...
    const updateCall = calls.find((call) => call.method === 'PATCH' && call.path === '/api/v1/client-access-groups/group-1');
    expect(updateCall?.body).toMatchObject({
      external_egress_profile_id: 'provider-1',
      policy_json: {
        access_mode: 'instance_default',
        egress_mode: 'default',
        egress_node_id: '',
