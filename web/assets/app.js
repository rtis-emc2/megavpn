(() => {
  ...
  function diffSpecSummary(currentSpec, previousSpec) {
    ...
    const keys = new Set([...Object.keys(currentEntries), ...Object.keys(previousEntries)]);
    const changed = Array.from(keys).filter((key) => currentEntries[key] !== previousEntries[key]).sort();
  ...
  async function renderRevisions() {
    ...
    try {
      ...
      const rows = (revisions || []).map((revision, index, list) => {
        ...
        summary: `${Object.keys(revision.spec || {}).length} spec keys · ${Array.isArray(revision.validation_errors) ? revision.validation_errors.length : 0} issues · ${diff.count} changed paths`,
      ...
      target.querySelectorAll('.revision-preview-btn').forEach((button) => {
        button.addEventListener('click', () => {
          ...
          if (row) openRevisionPreview(selectedInstance, row);
        });
      });
      target.querySelectorAll('.revision-rollback-btn').forEach((button) => {
        button.addEventListener('click', () => {
          const row = rows.find((item) => item.revision_id === button.dataset.revisionId);
          if (row) openRevisionRollbackModal(selectedInstance, row);
        });
      });
    } catch (err) {
      if (state.page !== 'revisions' || state.revisionsInstanceID !== selectedInstanceID) return;
  ...
  function openRevisionPreview(instance, row) {
    ...
    const closeBtn = document.getElementById('revisionPreviewCloseBtn');
    if (closeBtn) closeBtn.addEventListener('click', closeModal);
    const rollbackBtn = document.getElementById('revisionPreviewRollbackBtn');
    if (rollbackBtn) rollbackBtn.addEventListener('click', () => submitRevisionRollback(instance, row, false, rollbackBtn));
    const rollbackApplyBtn = document.getElementById('revisionPreviewRollbackApplyBtn');
    if (rollbackApplyBtn) rollbackApplyBtn.addEventListener('click', () => submitRevisionRollback(instance, row, true, rollbackApplyBtn));
  }
  ...
  function openRevisionRollbackModal(instance, row) {
    ...
    const rollbackBtn = document.getElementById('confirmRollbackRevisionBtn');
    if (rollbackBtn) rollbackBtn.addEventListener('click', () => submitRevisionRollback(instance, row, false, rollbackBtn));
    const rollbackApplyBtn = document.getElementById('confirmRollbackApplyRevisionBtn');
    if (rollbackApplyBtn) rollbackApplyBtn.addEventListener('click', () => submitRevisionRollback(instance, row, true, rollbackApplyBtn));
  }
  ...
  async function submitRevisionRollback(instance, row, applyAfterSave, button) {
    ...
    try {
      ...
      if (button) {
        ...
        button.textContent = button.dataset.originalLabel || 'Rollback';
      }
    }
  }
