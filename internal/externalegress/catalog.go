...
var protocolCatalog = []ProtocolDefinition{
	...
	{Code: "shadowsocks", Label: "Shadowsocks", Category: "proxy_tunnel", RuntimeSupport: RuntimePreview, ImportFormats: []string{"structured", "url", "json"}, CredentialModes: []string{"password"}, Transports: []string{"tcp", "udp"}, Notes: "Profile validation is available; node TUN materialization remains gated."},
	{Code: "vless", Label: "VLESS", Category: "proxy_tunnel", RuntimeSupport: RuntimePreview, ImportFormats: []string{"url", "json", "structured"}, CredentialModes: []string{"uuid", "certificate"}, Transports: []string{"tcp", "ws", "grpc", "reality"}, Notes: "Profile validation is available; node TUN materialization remains gated."},
	...
	{Code: "l2tp_ipsec", Label: "L2TP over IPsec", Category: "tunnel", RuntimeSupport: RuntimePlanned, ImportFormats: []string{"structured"}, CredentialModes: []string{"username_password", "psk", "certificate_private_key"}, Transports: []string{"udp", "ipsec"}, Notes: "Client-mode runtime driver is planned and activation is blocked until available."},
