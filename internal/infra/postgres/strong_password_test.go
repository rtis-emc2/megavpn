...
func TestRandomStrongPasswordShape(t *testing.T) {
	...
	if !passwordContainsAny(password, "abcdefghijklmnopqrstuvwxyz") {
	...
	if !passwordContainsAny(password, "0123456789") {
		t.Fatalf("password does not include a digit: %q", password)
	}
	if !passwordContainsAny(password, "!#$%&()*+,-.:=?@_~") {
		t.Fatalf("password does not include a symbol: %q", password)
	}
}

func passwordContainsAny(value, candidates string) bool {
