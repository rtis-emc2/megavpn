...
func (s *Store) RemoveClientAccessGroupMember(ctx context.Context, groupID, clientID string) (domain.ClientAccessGroupMembershipResult, error) {
	...
	if strings.EqualFold(group.Status, "deleted") || group.DeletedAt != nil {
		return domain.ClientAccessGroupMembershipResult{}, domain.ErrClientAccessGroupNotFound
	}
...
func (s *Store) bulkClientAccessGroupMembers(ctx context.Context, groupID string, req domain.ClientAccessGroupMembershipRequest, forceMove bool) (domain.ClientAccessGroupMembershipResult, error) {
	...
	if req.AllFiltered {
		...
		clientIDs = mergeClientIDs(clientIDs, filteredIDs)
	}
	result := domain.ClientAccessGroupMembershipResult{
		GroupID:     group.ID,
		GroupKey:    group.GroupKey,
		...
		for _, clientID := range clientIDs {
			...
			if member.ServiceAccessID != "" {
				if err := s.markServiceAccessClientAccessGroup(ctx, member.ServiceAccessID, group, desiredHash); err != nil {
					failures = append(failures, domain.ClientAccessGroupMembershipFailure{ClientID: clientID, Error: fmt.Sprintf("instance %s metadata: %s", instance.ID, err.Error())})
					continue
				}
			}
			if skipped {
...
func (s *Store) resolveVLESSAccessGroupInstances(ctx context.Context, group domain.ClientAccessGroup, requireExternalRouting bool) ([]domain.Instance, error) {
	candidates, err := s.listXrayVLESSAccessPolicyInstances(ctx)
	if err != nil {
		return nil, err
	}
	var catalog vlessAccessPolicyCatalogSnapshot
	...
	for _, instance := range candidates {
		...
		if requireExternalRouting {
			...
			spec, _, err = s.syncXrayVLESSAccessPolicySpec(ctx, instance, instance.Spec, catalog)
		}
		if err != nil {
			return nil, err
		}
		...
		for _, candidate := range xrayVLESSGroups(spec) {
			if candidate.Key == group.GroupKey && vlessAccessPolicyIsActive(candidate) {
				out = append(out, instance)
				break
			}
		}
...
func (s *Store) getClientAccessGroupByServiceKey(ctx context.Context, serviceCode, groupKey string) (domain.ClientAccessGroup, error) {
	serviceCode = normalizeClientAccessGroupServiceCode(serviceCode)
	groupKey = normalizeVLESSAccessPolicyKey(groupKey)
	row := s.db.QueryRow(ctx, `select
			id::text,service_code,group_key,display_name,coalesce(description,''),status,policy_json,external_egress_profile_id::text,
			scope_mode,auto_apply_new_instances,
			(select count(*)::int from client_access_group_memberships m where m.group_id=client_access_groups.id and m.status in ('active','disabled')),
...
func normalizeClientAccessGroupInput(input domain.ClientAccessGroupInput, create bool) (normalizedClientAccessGroupInput, error) {
	...
	out.GroupKey = normalizeVLESSAccessPolicyKey(out.GroupKey)
	...
	if len(out.DisplayName) > 160 {
		return out, fmt.Errorf("display_name must not exceed 160 bytes")
	}
	if len(out.Description) > 4000 {
		return out, fmt.Errorf("description must not exceed 4000 bytes")
	}
	if out.Status == "" {
		out.Status = "active"
	}
...
func (s *Store) validateClientAccessGroupExternalEgressDeploymentsForScope(ctx context.Context, group domain.ClientAccessGroup, include, exclude map[string]struct{}) error {
	...
	instances, err := s.listXrayVLESSAccessPolicyInstances(ctx)
	if err != nil {
		return err
	}
...
func normalizeVLESSAccessPolicyKey(raw string) string {
