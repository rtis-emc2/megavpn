...
func (s *Store) CreateSecretRef(ctx context.Context, secretType string, rawValue []byte, meta map[string]any) (domain.SecretRef, error) {
	ref, err := s.createSecretRef(ctx, s.db, secretType, rawValue, meta)
	if err != nil {
...
type secretRefExecutor interface {
	Exec(context.Context, string, ...any) (pgconn.CommandTag, error)
}

func (s *Store) createSecretRef(ctx context.Context, executor secretRefExecutor, secretType string, rawValue []byte, meta map[string]any) (domain.SecretRef, error) {
	...
	if _, err := executor.Exec(ctx, `insert into secret_refs(id,secret_type,ciphertext,key_version,nonce,meta_json,created_at) values($1,$2,$3,$4,$5,$6,$7)`,
