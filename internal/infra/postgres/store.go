...
func (s *Store) ListServiceDefinitions(ctx context.Context) ([]domain.ServiceDefinition, error) {
	rows, err := s.db.Query(ctx, `select id,code,name,category,tier,supports_accounts,supports_artifacts,coalesce(supports_install,false),coalesce(supports_instances,true),enabled,created_at,updated_at from service_definitions order by tier, code`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&x.ID, &x.Code, &x.Name, &x.Category, &x.Tier, &x.SupportsAccounts, &x.SupportsArtifacts, &x.SupportsInstall, &x.SupportsInstances, &x.Enabled, &x.CreatedAt, &x.UpdatedAt); err != nil {
			return nil, err
		}
		originalCode := x.Code
