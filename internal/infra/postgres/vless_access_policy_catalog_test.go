...
func TestShouldQueueVLESSAccessPolicyApply(t *testing.T) {
	tests := []struct {
		name    string
		status  string
		enabled bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, _ := shouldQueueXrayAccessPolicyApply(domain.Instance{Status: tt.status, Enabled: tt.enabled})
			if got != tt.want {
				t.Fatalf("shouldQueueXrayAccessPolicyApply() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestClearResolvedXrayVLESSGroupEgress(t *testing.T) {
	group := map[string]any{
		...
		"egress_resolved_by": "control-plane:vless-access-policy-sync",
	}
...
func TestSpecWithVLESSAccessPolicySnapshotDropsInactiveManagedGroups(t *testing.T) {
	spec := map[string]any{
	...
	catalog := vlessAccessPolicyCatalogSnapshot{
	...
	enriched, changed := specWithVLESSAccessPolicyCatalogSnapshot(spec, catalog)
	if !changed {
		t.Fatal("specWithVLESSAccessPolicyCatalogSnapshot() changed = false, want true")
	}
