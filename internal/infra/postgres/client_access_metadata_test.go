...
func TestResolveXrayVLESSGroupEgressWithResolverMaterializesSelectedEgress(t *testing.T) {
	...
	changed, err := resolveXrayVLESSGroupEgressWithResolver(
		...
		"control-plane:vless-access-policy-sync",
	)
	if err != nil {
	...
	if got := firstString(group["egress_resolved_by"]); got != "control-plane:vless-access-policy-sync" {
		t.Fatalf("egress_resolved_by = %q, want control-plane:vless-access-policy-sync", got)
	}
