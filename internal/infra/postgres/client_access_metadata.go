...
func (s *Store) ensureXrayProvisioningGroups(ctx context.Context, instanceID string, spec map[string]any) (map[string]any, error) {
	...
	catalog, err := s.loadVLESSAccessPolicyCatalogSnapshot(ctx)
	if err != nil {
		return nil, err
	}
	enriched, changed, err := s.syncXrayVLESSAccessPolicySpec(ctx, instance, spec, catalog)
	if err != nil {
		return nil, fmt.Errorf("prepare VLESS access policies for service instance: %w", err)
	}
	if !changed {
		return enriched, nil
	}
	...
	if err != nil {
		return nil, fmt.Errorf("materialize VLESS access policies for service instance: %w", err)
	}
	status, _, validationErrors := s.validateInstanceRevisionSpec(ctx, instance, materialized)
	if !in(status, "validated", "applied") {
		return nil, fmt.Errorf("VLESS access policy revision is not apply-ready; status=%s errors=%v", status, validationErrors)
	}
	revision, err := s.ReplaceInstanceSpec(ctx, instanceID, "system:vless-groups", materialized)
	if err != nil {
		return nil, fmt.Errorf("sync VLESS access policies into service instance: %w", err)
	}
	if !in(strings.TrimSpace(revision.Status), "validated", "applied") {
		return nil, fmt.Errorf("VLESS access policy revision is not apply-ready; status=%s errors=%v", strings.TrimSpace(revision.Status), revision.ValidationErrors)
	}
	return revision.Spec, nil
}

func (s *Store) listXrayVLESSAccessPolicyInstances(ctx context.Context) ([]domain.Instance, error) {
...
func (s *Store) syncXrayVLESSAccessPolicySpec(ctx context.Context, instance domain.Instance, spec map[string]any, catalog vlessAccessPolicyCatalogSnapshot) (map[string]any, bool, error) {
	original := cloneMap(spec)
	enriched, _ := specWithVLESSAccessPolicyCatalogSnapshot(spec, catalog)
	if _, err := s.resolveXrayDefaultEgress(ctx, instance, enriched, "control-plane:vless-access-policy-sync"); err != nil {
...
func shouldQueueXrayAccessPolicyApply(instance domain.Instance) (bool, string) {
	if !instance.Enabled {
...
func (s *Store) resolveXrayVLESSGroupEgress(ctx context.Context, instance domain.Instance, spec map[string]any) (bool, error) {
	return resolveXrayVLESSGroupEgressWithResolver(ctx, instance, spec, s.ResolveXrayVLESSEgress, "control-plane:vless-access-policy-sync")
}
...
func resolveXrayVLESSGroupEgressWithResolver(ctx context.Context, instance domain.Instance, spec map[string]any, resolver xrayVLESSEgressResolver, resolvedBy string) (bool, error) {
	...
	if resolvedBy == "" {
		resolvedBy = "control-plane:vless-access-policy-sync"
	}
	changed := false
...
type vlessAccessPolicyCatalogSnapshot struct {
	active  []domain.VLESSAccessPolicy
	catalog []domain.VLESSAccessPolicy
	managed map[string]struct{}
}

func (s *Store) loadVLESSAccessPolicyCatalogSnapshot(ctx context.Context) (vlessAccessPolicyCatalogSnapshot, error) {
	groups, err := s.ListClientAccessGroups(ctx, "vless")
	if err != nil {
		return vlessAccessPolicyCatalogSnapshot{}, err
	}
	active := make([]domain.VLESSAccessPolicy, 0, len(groups))
	catalog := make([]domain.VLESSAccessPolicy, 0, len(groups))
	for _, group := range groups {
		policy, err := accesspolicy.DecodeVLESS(group)
		if err != nil {
			return vlessAccessPolicyCatalogSnapshot{}, fmt.Errorf("decode VLESS access group %q: %w", group.GroupKey, err)
		}
		catalog = append(catalog, policy)
		if strings.EqualFold(strings.TrimSpace(group.Status), "active") {
			active = append(active, policy)
		}
	}
	snapshot := vlessAccessPolicyCatalogSnapshot{
		active:  active,
		catalog: catalog,
...
func specWithVLESSAccessPolicyCatalogSnapshot(spec map[string]any, catalog vlessAccessPolicyCatalogSnapshot) (map[string]any, bool) {
	...
	for _, template := range catalog.active {
		add(vlessAccessPolicySpecGroup(template))
	}
...
func vlessAccessPolicySpecGroup(template domain.VLESSAccessPolicy) map[string]any {
	...
	if len(template.Rules) > 0 {
		group["rules"] = cloneVLESSAccessPolicyRules(template.Rules)
	}
	if len(template.ExtraRules) > 0 {
		group["extra_rules"] = cloneVLESSAccessPolicyRules(template.ExtraRules)
	}
	return group
}

func cloneVLESSAccessPolicyRules(rules []map[string]any) []any {
