...
func (s *Store) CreateBackhaulLink(ctx context.Context, link domain.BackhaulLink) (domain.BackhaulLink, error) {
	...
	createdSecretRefIDs := make([]string, 0)
	...
	for idx, driver := range drivers {
		...
		transport, err := s.buildBackhaulTransport(ctx, tx, link, def, endpointHost, transportCIDR, idx)
		if err != nil {
			return domain.BackhaulLink{}, err
		}
		...
		if _, err := tx.Exec(ctx, `insert into backhaul_transports(
			id,link_id,driver,priority,status,endpoint_host,endpoint_port,protocol,interface_name,tunnel_cidr,ingress_address,egress_address,config_json,health_json,created_at,updated_at
			...
			transport.InterfaceName, transport.TunnelCIDR, transport.IngressAddress, transport.EgressAddress, mustJSON(transport.Config), mustJSON(transport.Health), now, now); err != nil {
			return domain.BackhaulLink{}, err
		}
		if err := insertBackhaulTransportSecretRefs(ctx, tx, transport); err != nil {
			return domain.BackhaulLink{}, err
		}
		for _, purpose := range sortedBackhaulSecretRefNames(transport.SecretRefs) {
			createdSecretRefIDs = append(createdSecretRefIDs, strings.TrimSpace(stringify(transport.SecretRefs[purpose])))
		}
	...
	for _, secretRefID := range createdSecretRefIDs {
		_, _ = s.CreateAudit(ctx, "system", "secret_ref.create", "secret_ref", &secretRefID, "backhaul secret ref stored")
	}
...
func (s *Store) SetBackhaulRouteEnabled(ctx context.Context, linkID string, enabled bool) (domain.BackhaulLink, []domain.Job, error) {
	if _, err := s.RecoverStaleJobLeases(ctx); err != nil {
		return domain.BackhaulLink{}, nil, err
	}
	link, err := s.GetBackhaulLink(ctx, linkID)
	...
	if enabled {
		...
		_, _ = s.CreateAudit(ctx, "system", "backhaul.route.enable", "backhaul", &link.ID, "managed backhaul route enable queued")
		return link, jobs, nil
	}
...
func buildBackhaulRouteDisablePayload(link domain.BackhaulLink, transport domain.BackhaulTransport, role, batchID string) map[string]any {
	payload := buildBackhaulCleanupPayload(link, transport, role, batchID)
	delete(payload, "delete_batch_id")
	payload["route_disable_batch_id"] = batchID
	payload["route_action"] = "disable"
	return payload
}
...
func (s *Store) buildBackhaulTransport(ctx context.Context, tx pgx.Tx, link domain.BackhaulLink, def backhaul.DriverDefinition, endpointHost, tunnelCIDR string, idx int) (domain.BackhaulTransport, error) {
	...
	case backhaul.DriverWireGuard:
		...
		ingressRef, err := s.createSecretRef(ctx, tx, "private_key", []byte(ingressPriv), map[string]any{"scope": "backhaul", "link_id": link.ID, "driver": def.Code, "role": "ingress"})
		if err != nil {
			return domain.BackhaulTransport{}, err
		}
		egressRef, err := s.createSecretRef(ctx, tx, "private_key", []byte(egressPriv), map[string]any{"scope": "backhaul", "link_id": link.ID, "driver": def.Code, "role": "egress"})
		if err != nil {
			return domain.BackhaulTransport{}, err
		}
	...
	case backhaul.DriverOpenVPNUDP, backhaul.DriverOpenVPNTCP443:
		...
		ref, err := s.createSecretRef(ctx, tx, "psk", []byte(key), map[string]any{"scope": "backhaul", "link_id": link.ID, "driver": def.Code, "material": "openvpn_static_key"})
		if err != nil {
			return domain.BackhaulTransport{}, err
		}
	...
	case backhaul.DriverIPSecL2TP, backhaul.DriverIKEv2:
		...
		ref, err := s.createSecretRef(ctx, tx, "psk", []byte(psk), map[string]any{"scope": "backhaul", "link_id": link.ID, "driver": def.Code, "material": "ipsec_psk"})
		if err != nil {
			return domain.BackhaulTransport{}, err
		}
	...
	default:
		uuid := id.New()
		ref, err := s.createSecretRef(ctx, tx, "uuid", []byte(uuid), map[string]any{"scope": "backhaul", "link_id": link.ID, "driver": def.Code, "material": "xray_uuid"})
		if err != nil {
			return domain.BackhaulTransport{}, err
		}
		...
		if def.Code == backhaul.DriverXrayReality {
			...
			privateRef, err := s.createSecretRef(ctx, tx, "private_key", []byte(privateKey), map[string]any{"scope": "backhaul", "link_id": link.ID, "driver": def.Code, "material": "xray_reality_private_key"})
			if err != nil {
				return domain.BackhaulTransport{}, err
			}
...
func (s *Store) listBackhaulTransports(ctx context.Context, linkID string) ([]domain.BackhaulTransport, error) {
	rows, err := s.db.Query(ctx, `select
		...
		config_json,
		coalesce((
			select jsonb_object_agg(secret.purpose, secret.secret_ref_id::text order by secret.purpose)
			from backhaul_transport_secrets secret
			where secret.transport_id = backhaul_transports.id
		), '{}'::jsonb) as secret_refs_json,
		health_json,
		applied_ingress_at,
		applied_egress_at,
		last_error,
...
func scanBackhaulTransport(row pgx.Row) (domain.BackhaulTransport, error) {
	...
	if err := decodeJSONField(secretRefsRaw, &transport.SecretRefs, "backhaul_transport_secrets"); err != nil {
		return domain.BackhaulTransport{}, err
	}
...
func insertBackhaulTransportSecretRefs(ctx context.Context, tx pgx.Tx, transport domain.BackhaulTransport) error {
	for _, purpose := range sortedBackhaulSecretRefNames(transport.SecretRefs) {
		secretRefID := strings.TrimSpace(stringify(transport.SecretRefs[purpose]))
		if secretRefID == "" {
			return fmt.Errorf("backhaul transport secret %q has an empty reference", purpose)
		}
		if _, err := tx.Exec(ctx, `insert into backhaul_transport_secrets(
			transport_id,purpose,secret_ref_id,created_at,updated_at
		) values($1,$2,$3,$4,$5)`, transport.ID, purpose, secretRefID, transport.CreatedAt, transport.UpdatedAt); err != nil {
			return fmt.Errorf("link backhaul transport secret %q: %w", purpose, err)
		}
	}
	return nil
}
...
func renderBackhaulIngressStopScript(slug, iface, transportStopCmd, sourceAddress, routingTable string) string {
	return renderBackhaulStopScriptWithExtra(slug, iface, transportStopCmd, renderBackhaulIngressSourcePolicyStopLines(sourceAddress, routingTable), "ingress-snat")
}

func renderBackhaulStopScript(slug, iface, transportStopCmd string, labels ...string) string {
	return renderBackhaulStopScriptWithExtra(slug, iface, transportStopCmd, nil, labels...)
}

func renderBackhaulStopScriptWithExtra(slug, iface, transportStopCmd string, extraLines []string, labels ...string) string {
	if len(labels) == 0 {
...
func renderBackhaulIngressSourcePolicyStopLines(sourceAddress, routingTable string) []string {
	...
	return []string{
		...
		"  ip route flush table " + shellQuotePG(routingTable) + " >/dev/null 2>&1 || true",
		"fi",
	}
}
