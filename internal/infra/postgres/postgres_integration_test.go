...
func TestPostgresIntegrationClientAccessGroupsVLESSBulkMaterialization(t *testing.T) {
	...
	if _, err := store.UpdateClientAccessGroup(ctx, outUSASF.ID, domain.ClientAccessGroupInput{
		...
		Status:                "active",
		PolicyJSON:            []byte(`{"access_mode":"local_breakout","egress_mode":"local_breakout","outbound_tag":"direct","rules":[{"type":"field","domain":["geosite:private"],"outbound_tag":"direct"}]}`),
		ScopeMode:             "selected_instances",
...
func TestPostgresIntegrationBackhaulRouteToggleRefreshesRoutePolicy(t *testing.T) {
	...
	assertPostgresCount(t, ctx, store, `select count(*) from backhaul_transport_secrets where transport_id=$1`, len(link.Transports[0].SecretRefs), link.Transports[0].ID)
...
func TestPostgresIntegrationDeleteClientRemovesProvisioningRows(t *testing.T) {
	...
	delivery, err := store.CreateClientEmailDelivery(ctx, domain.ClientEmailDelivery{
	...
	assertPostgresCount(t, ctx, store, `select count(*) from client_email_delivery_artifacts where delivery_id=$1 and artifact_id=$2`, 1, delivery.ID, artifact.ID)
	assertPostgresCount(t, ctx, store, `select count(*) from client_email_delivery_share_links where delivery_id=$1 and share_link_id=$2`, 1, delivery.ID, share.ID)
	otherClient, err := store.CreateClient(ctx, domain.Client{
		Username:    "it-cleanup-other-" + suffix,
		DisplayName: "Integration Other Client",
		Email:       "it-cleanup-other-" + suffix + "@example.invalid",
	})
	if err != nil {
		t.Fatalf("create other client: %v", err)
	}
	if _, err := store.CreateClientEmailDelivery(ctx, domain.ClientEmailDelivery{
		ClientAccountID: otherClient.ID,
		Email:           otherClient.Email,
		Subject:         "Cross-client attachment must fail",
		Status:          "queued",
		ArtifactIDs:     []string{artifact.ID},
	}); err == nil {
		t.Fatal("cross-client email artifact attachment should fail")
	}
	assertPostgresCount(t, ctx, store, `select count(*) from client_email_deliveries where client_account_id=$1`, 0, otherClient.ID)
...
func TestPostgresIntegrationForceRetireLostNodeCleansControlPlaneState(t *testing.T) {
	...
	backhaulTransportID := id.New()
	if _, err := store.db.Exec(ctx, `insert into backhaul_transports(id,link_id,driver)
		values($1,$2,'wireguard')`, backhaulTransportID, backhaulID); err != nil {
		t.Fatalf("insert backhaul transport: %v", err)
	}
	if _, err := store.db.Exec(ctx, `insert into backhaul_transport_secrets(transport_id,purpose,secret_ref_id)
		values($1,'private_key_secret_ref_id',$2)`, backhaulTransportID, backhaulSecret.ID); err != nil {
		t.Fatalf("insert backhaul transport secret mapping: %v", err)
	}
	...
	assertPostgresCount(t, ctx, store, `select count(*) from backhaul_transport_secrets where transport_id=$1`, 0, backhaulTransportID)
...
func clientAccessGroupByKey(t *testing.T, ctx context.Context, store *Store, serviceCode, groupKey string) domain.ClientAccessGroup {
	...
	for _, group := range groups {
		if group.ServiceCode == normalizeClientAccessGroupServiceCode(serviceCode) && group.GroupKey == normalizeVLESSAccessPolicyKey(groupKey) {
			return group
		}
	}
