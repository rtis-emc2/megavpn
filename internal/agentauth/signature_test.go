...
func TestNewNonceFailsClosedWhenEntropyIsUnavailable(t *testing.T) {
	t.Parallel()

	nonce, err := newNonce(failingNonceReader{})
	if err == nil {
		t.Fatal("newNonce() error = nil, want entropy failure")
	}
	if nonce != "" {
		t.Fatalf("newNonce() nonce = %q, want empty value on failure", nonce)
	}
}
