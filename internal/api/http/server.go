...
type Store interface {
	...
	ClearNodeStalePendingRotation(context.Context, string, domain.NodeStaleRotationClearInput) (domain.NodeStaleRotationClearResult, error)
...
func New(log *slog.Logger, store Store, opts Options) nethttp.Handler {
	...
	protected("POST /api/v1/nodes/{id}/access-methods/ssh", "node.bootstrap", s.createNodeSSHAccessMethod)
	...
	protected("DELETE /api/v1/nodes/{id}/enrollment-tokens/{token_id}", "node.bootstrap", s.revokeNodeEnrollmentToken)
