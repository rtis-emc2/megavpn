...
const publicBearerResourceUnavailable = "requested resource is unavailable"
...
func (s *Server) publicShareDownload(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeErr(w, 404, publicBearerResourceUnavailable)
		return
	}
...
func (s *Server) agentBinaryArtifactDownload(w http.ResponseWriter, r *http.Request) {
	...
	if !s.authorizeAgentNode(r, nodeID) {
		if err := s.store.RecordAgentAuthFailure(r.Context(), nodeID, "binary artifact download unauthorized"); err != nil {
			s.logPersistenceFailure("agent.auth_failure.record", err, "node_id", nodeID, "endpoint", "binary_artifact_download")
		}
...
func (s *Server) serveBinaryArtifactDownload(w http.ResponseWriter, r *http.Request, artifact domain.BinaryArtifact, agentToken string) error {
	...
	if !setSignedAgentResponseHeaders(w, r, agentToken, bodyHash) {
		return fmt.Errorf("sign binary artifact response")
	}
	http.ServeFile(w, r, path)
	return nil
}
