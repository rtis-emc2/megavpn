...
func (s *Server) upsertServicePack(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	authCtx, ok := authFromRequest(r)
	if ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "service_pack.upsert", "service_pack", nil, "service pack template upserted: "+updated.Key)
	}
...
func (s *Server) setServicePackStatus(status string) nethttp.HandlerFunc {
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		authCtx, ok := authFromRequest(r)
		if ok {
			s.auditBestEffort(r.Context(), &authCtx.User.ID, "service_pack."+status, "service_pack", nil, "service pack template "+status+": "+updated.Key)
		}
		writeJSON(w, 200, updated)
	}
}
...
func (s *Server) createServicePackInstances(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	for _, selected := range selectedComponents {
		...
		for attempt := 0; attempt < 16; attempt++ {
			...
			if strings.EqualFold(componentService, driver.XrayCore) || strings.EqualFold(componentService, "xray") {
				vlessGroups, err := s.availableVLESSAccessPolicies(r.Context())
				if err != nil {
					writeErr(w, 409, "vless access policy preflight failed: "+err.Error())
					return
				}
				if err := ensureVLESSDefaultGroup(spec, vlessGroups); err != nil {
					writeErr(w, 409, "vless access policy preflight failed: "+err.Error())
					return
				}
			}
	...
	if ok {
		if len(created) == 0 && len(existing) > 0 {
			s.auditBestEffort(r.Context(), &authCtx.User.ID, "service_pack.reuse", "service_pack", nil, "service pack already existed: "+pack.Key)
		} else {
			s.auditBestEffort(r.Context(), &authCtx.User.ID, "service_pack.create", "service_pack", nil, "service pack created: "+pack.Key)
		}
	}
	statusCode := 201
	statusText := "ok"
