...
func (s *Server) login(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if err != nil {
		s.auditBestEffort(r.Context(), nil, "auth.login_failed", "platform_user", nil, "operator login failed")
		writeErr(w, 401, "invalid credentials")
		return
	}
	if record.User.Status != "active" || !authn.VerifyPassword(req.Password, record.PasswordHash) {
		s.auditBestEffort(r.Context(), &record.User.ID, "auth.login_failed", "platform_user", &record.User.ID, "operator login failed")
		writeErr(w, 401, "invalid credentials")
		return
	}
	...
	if err := s.store.TouchPlatformUserLogin(r.Context(), record.User.ID); err != nil && s.log != nil {
		s.log.Warn("platform user login timestamp update failed", "user_id", record.User.ID, "error", err)
	}
	if err := s.auditRequired(r.Context(), &record.User.ID, "auth.login", "platform_user", &record.User.ID, "operator logged in"); err != nil {
		if revokeErr := s.store.RevokeUserSession(r.Context(), sess.ID); revokeErr != nil && s.log != nil {
			s.log.Error("session rollback after audit failure failed", "session_id", sess.ID, "error", revokeErr)
		}
		writeErr(w, 503, "login audit persistence failed")
		return
	}
...
func (s *Server) logout(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	s.auditBestEffort(r.Context(), &authCtx.User.ID, "auth.logout", "platform_user", &authCtx.User.ID, "operator logged out")
	nethttp.SetCookie(w, &nethttp.Cookie{
		Name:     s.sessionCookieName,
		Value:    "",
		Path:     "/",
