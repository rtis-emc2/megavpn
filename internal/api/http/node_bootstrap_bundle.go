...
func (s *Server) getNodeBootstrapBundle(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		if err := s.auditRequired(r.Context(), &authCtx.User.ID, "node.bootstrap_bundle.reveal", "node", &nodeID, "manual bootstrap bundle revealed"); err != nil {
			if s.log != nil {
				s.log.Error("bootstrap bundle reveal blocked by audit failure", "node_id", nodeID, "run_id", runID, "error", err)
			}
			writeErr(w, 503, "audit persistence is unavailable")
			return
		}
	}
