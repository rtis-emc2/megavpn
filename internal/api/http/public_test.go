...
func TestServeBinaryArtifactDownloadFailsClosedWithoutSigningToken(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	path := filepath.Join(root, "megavpn-agent")
	content := []byte("signed runtime artifact")
	if err := os.WriteFile(path, content, 0o600); err != nil {
		t.Fatalf("write binary artifact: %v", err)
	}
	server := &Server{artifactRoot: root}
	req := httptest.NewRequest(http.MethodGet, "/agent/binary-artifacts/artifact-1/download", nil)
	recorder := httptest.NewRecorder()
	err := server.serveBinaryArtifactDownload(recorder, req, domain.BinaryArtifact{
		Status:      "active",
		StoragePath: path,
		SHA256:      agentauth.BodyHash(content),
	}, "")
	if err == nil {
		t.Fatal("unsigned binary artifact response must be rejected")
	}
	if recorder.Body.Len() != 0 {
		t.Fatalf("unsigned binary artifact leaked %d response bytes", recorder.Body.Len())
	}
	if recorder.Header().Get(agentauth.HeaderSignature) != "" {
		t.Fatal("unsigned binary artifact response unexpectedly has a signature")
	}
}
