...
func (s *Server) createNodeSSHTerminalSession(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if err := s.auditRequired(r.Context(), &authCtx.User.ID, "node.ssh_terminal.ticket", "node", &nodeID, "web ssh terminal ticket created"); err != nil {
		s.terminalSessions.consume(ticket.ID, time.Now().UTC())
		s.logPersistenceFailure("node.ssh_terminal.ticket.audit", err, "node_id", nodeID, "user_id", authCtx.User.ID)
		writeErr(w, 503, "ssh terminal audit persistence failed")
		return
	}
...
func (s *Server) runNodeSSHTerminal(parent context.Context, ws *terminalWebSocket, nodeID string, authCtx domain.AuthContext) error {
	...
	if err := s.auditRequired(ctx, &authCtx.User.ID, "node.ssh_terminal.start", "node", &nodeID, "web ssh terminal started"); err != nil {
		if cmd.Process != nil {
			_ = cmd.Process.Kill()
		}
		_ = cmd.Wait()
		return fmt.Errorf("ssh terminal audit persistence failed: %w", err)
	}
	...
	defer auditCancel()
	s.auditBestEffort(auditCtx, &authCtx.User.ID, "node.ssh_terminal.end", "node", &nodeID, "web ssh terminal closed")
	return nil
}
