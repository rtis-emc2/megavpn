...
func (s *Server) createFirewallPolicy(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.policy.create", "firewall", &created.ID, "firewall policy created: "+created.Key)
	}
...
func (s *Server) updateFirewallPolicy(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.policy.update", "firewall", &updated.ID, "firewall policy updated: "+updated.Key)
	}
...
func (s *Server) deleteFirewallPolicy(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.policy.delete", "firewall", &deleted.ID, "firewall policy deleted: "+deleted.Key)
	}
...
func (s *Server) createFirewallAddressList(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.address_list.create", "firewall", &created.ID, "firewall address-list created: "+created.Key)
	}
	writeJSON(w, 201, created)
}
...
func (s *Server) updateFirewallAddressList(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.address_list.update", "firewall", &updated.ID, "firewall address-list updated: "+updated.Key)
	}
	writeJSON(w, 200, updated)
}
...
func (s *Server) deleteFirewallAddressList(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.address_list.delete", "firewall", &deleted.ID, "firewall address-list deleted: "+deleted.Key)
	}
	writeJSON(w, 200, deleted)
}
...
func (s *Server) createFirewallAddressEntry(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.address_entry.create", "firewall", &created.ID, "firewall address entry created")
	}
	writeJSON(w, 201, created)
}
...
func (s *Server) updateFirewallAddressEntry(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.address_entry.update", "firewall", &updated.ID, "firewall address entry updated")
	}
	writeJSON(w, 200, updated)
}
...
func (s *Server) deleteFirewallAddressEntry(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.address_entry.delete", "firewall", &deleted.ID, "firewall address entry deleted")
	}
	writeJSON(w, 200, deleted)
}
...
func (s *Server) updateFirewallManagementSettings(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.management_settings.update", "firewall", nil, "firewall management source CIDRs updated")
	}
	writeJSON(w, 200, updated)
}
...
func (s *Server) createFirewallRule(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.rule.create", "firewall", &created.ID, "firewall rule created")
	}
	writeJSON(w, 201, created)
}
...
func (s *Server) updateFirewallRule(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.rule.update", "firewall", &updated.ID, "firewall rule updated")
	}
	writeJSON(w, 200, updated)
}
...
func (s *Server) deleteFirewallRule(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.rule.delete", "firewall", &deleted.ID, "firewall rule deleted")
	}
	writeJSON(w, 200, deleted)
}
...
func (s *Server) applyNodeFirewallPolicy(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.apply", "node", &nodeID, "firewall apply queued")
	}
	writeJSON(w, 202, job)
}
...
func (s *Server) disableNodeFirewall(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.disable", "node", &nodeID, "firewall disable queued")
	}
	writeJSON(w, 202, job)
}
...
func (s *Server) previewNodeFirewallPolicy(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if authCtx, ok := authFromRequest(r); ok {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "firewall.preview", "node", &nodeID, "firewall preview queued")
	}
	writeJSON(w, 202, job)
}
