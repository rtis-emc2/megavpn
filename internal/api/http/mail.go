...
func (s *Server) sendPlatformMailTest(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.auditBestEffort(r.Context(), &authCtx.User.ID, "platform.mail_settings.test", "platform_mail_settings", nil, "mail settings test failed")
		s.logMailDeliveryFailure("platform mail test failed", err, "recipient", strings.TrimSpace(req.Email))
		writeJSON(w, 502, response{"status": "delivery_failed", "error": publicMailDeliveryFailure, "test": testPayload})
		return
	}
	s.auditBestEffort(r.Context(), &authCtx.User.ID, "platform.mail_settings.test", "platform_mail_settings", nil, "mail settings test sent")
...
func (s *Server) invitePlatformUser(w http.ResponseWriter, r *http.Request) {
	...
	if linkBase == "" {
		writeErr(w, 409, "an absolute HTTPS invite URL base is required before sending invitations")
		return
	}
	...
	if err != nil {
		s.logMailDeliveryFailure("platform user invite delivery failed", err, "user_id", user.ID)
		writeJSON(w, 502, response{"status": "delivery_failed", "user": user, "invite": invite, "error": publicMailDeliveryFailure, "invite_url": link})
		return
	}
...
func (s *Server) resendPlatformUserInvite(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.logMailDeliveryFailure("platform user invite resend failed", err, "user_id", user.ID)
		writeJSON(w, 502, response{"status": "delivery_failed", "user": user, "invite": invite, "error": publicMailDeliveryFailure, "invite_url": link})
		return
	}
...
func (s *Server) acceptPlatformInvite(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.store.TouchPlatformUserLogin(r.Context(), user.ID); err != nil {
		s.logPersistenceFailure("platform.user.login.touch", err, "user_id", user.ID)
	}
...
func (s *Server) deliverClientEmail(w http.ResponseWriter, r *http.Request) {
	...
	if req.CreateShareLink || len(artifacts) == 0 {
		publicBaseURL = validatedMailBaseURL(s.publicBaseURL)
		if publicBaseURL == "" {
			writeErr(w, 409, "an absolute HTTPS public base URL is required before publishing client access links")
			return
		}
	...
	body := buildClientAccessBody(clientRecord, req.Message, artifacts, shareLinks, attachmentNotes, publicBaseURL)
	htmlBody := buildClientAccessHTML(clientRecord, req.Message, artifacts, shareLinks, attachmentNotes, publicBaseURL)
	err = s.sendPlatformMail(r.Context(), settings, mail.Message{
		FromEmail:   settings.FromEmail,
		FromName:    settings.FromName,
		ReplyTo:     settings.ReplyToEmail,
	...
	if persistErr := s.store.UpdateClientEmailDeliveryStatus(r.Context(), delivery.ID, status, errText, map[string]any{
		"attachment_count": len(attachments),
		"share_link_count": len(shareLinks),
	}); persistErr != nil {
		s.logPersistenceFailure("client.email_delivery.status", persistErr, "client_id", clientID, "delivery_id", delivery.ID, "status", status)
		if err == nil {
			writeJSON(w, 503, response{"status": "delivery_state_unknown", "delivery": delivery, "error": "email sent but delivery status could not be persisted"})
			return
		}
	}
	if err != nil {
		s.logMailDeliveryFailure("client mail delivery failed", err, "client_id", clientID, "delivery_id", delivery.ID)
		writeJSON(w, 502, response{"status": status, "delivery": delivery, "error": publicMailDeliveryFailure})
		return
	}
	writeJSON(w, 200, response{"status": "ok", "delivery": delivery})
}

const publicMailDeliveryFailure = "mail delivery failed; inspect control-plane logs and delivery status"

func (s *Server) logMailDeliveryFailure(message string, err error, attrs ...any) {
...
<head>
  ...
  <style>
    @media only screen and (max-width:620px) {
      ...
      .mail-content { padding:20px 18px !important; }
      .mail-summary-cell { display:block !important; width:100% !important; padding:0 0 8px !important; }
      .mail-meta-label, .mail-meta-value { display:block !important; width:100% !important; padding-left:0 !important; padding-right:0 !important; }
      .mail-meta-label { padding-bottom:3px !important; border-bottom:0 !important; }
      .mail-action { display:block !important; width:100% !important; box-sizing:border-box !important; }
    }
  </style>
</head>
<body style="margin:0;padding:0;background:#eef2f7;color:#111827;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;">
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;border-collapse:collapse;background:#eef2f7;">
    <tr>
      <td class="mail-outer" align="center" style="padding:28px 14px;">
        <table class="mail-shell" role="presentation" width="680" cellspacing="0" cellpadding="0" style="width:100%;max-width:680px;border-collapse:collapse;">
          <tr>
            <td style="padding:0 4px 16px 4px;">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
                <tr>
                  <td width="58" valign="middle" style="width:58px;">
                    <div style="width:44px;height:44px;border-radius:8px;background:#b91c1c;color:#ffffff;font-weight:800;font-size:13px;line-height:44px;text-align:center;letter-spacing:0.04em;">NL</div>
                  </td>
                  <td valign="middle">
          ...
          <tr>
            <td style="background:#ffffff;border:1px solid #d9e2ec;border-radius:8px;overflow:hidden;">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
                <tr>
                  <td class="mail-content" style="padding:26px 30px 22px;border-bottom:1px solid #e5e7eb;background:#ffffff;">
                ...
                <tr>
                  <td class="mail-content" style="padding:24px 30px 28px;background:#ffffff;">
...
func buildMailBadges(badges []mailBadge) string {
	...
	for i, badge := range badges {
		...
		b.WriteString(`<td class="mail-summary-cell" valign="top" style="width:33.333%;padding:0 6px 12px 0;">`)
		b.WriteString(`<div style="border:1px solid #dbe4ee;background:#f8fafc;border-radius:8px;padding:13px;min-height:88px;">`)
...
func buildMailAction(action *mailAction) string {
	...
	safeURL := validatedMailURL(action.URL)
	if safeURL == "" {
		return ""
	}
  ...
  <tr>
    <td align="center" style="padding:18px;border:1px solid #fecdd3;background:#fff1f2;border-radius:8px;">
      <a class="mail-action" role="button" href="` + html.EscapeString(safeURL) + `" style="display:inline-block;background:#b91c1c;color:#ffffff;text-decoration:none;font-size:14px;font-weight:800;line-height:1;padding:13px 20px;border-radius:8px;">` + html.EscapeString(action.Label) + `</a>
      <div style="margin-top:12px;color:#64748b;font-size:12px;line-height:1.6;">` + html.EscapeString(hint) + `<br><span style="color:#334155;word-break:break-all;">` + html.EscapeString(safeURL) + `</span></div>
    </td>
  </tr>
</table>`
}
...
func buildMailSection(title, body string) string {
	...
	return `<div style="margin-top:16px;border:1px solid #e2e8f0;border-radius:8px;background:#ffffff;overflow:hidden;">
...
func buildMailNotice(text string) string {
	...
	return `<div style="margin-top:16px;padding:14px 16px;border:1px solid #fde68a;background:#fffbeb;border-radius:8px;color:#92400e;font-size:13px;line-height:1.6;">` + html.EscapeString(text) + `</div>`
}
...
func buildMailMetaTable(rows []mailMetaRow) string {
	...
	for _, row := range rows {
		b.WriteString(`<tr><td class="mail-meta-label" valign="top" width="34%" style="width:34%;padding:10px 14px 10px 0;border-bottom:1px solid #e5e7eb;color:#64748b;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:0.06em;line-height:1.45;">`)
		b.WriteString(html.EscapeString(row.Label))
		b.WriteString(`</td><td class="mail-meta-value" valign="top" width="66%" style="width:66%;padding:10px 0;border-bottom:1px solid #e5e7eb;color:#111827;font-size:13px;font-weight:700;line-height:1.45;word-break:break-word;">`)
...
func shareLinkPublicURL(link domain.ShareLink, publicBaseURL string) string {
	...
	base := validatedMailBaseURL(publicBaseURL)
	if base == "" {
		return ""
	}
	return strings.TrimRight(base, "/") + "/share/" + url.PathEscape(strings.TrimSpace(link.Token))
}

func validatedMailURL(raw string) string {
	parsed, err := url.Parse(strings.TrimSpace(raw))
	if err != nil || parsed.User != nil || parsed.Host == "" || (parsed.Scheme != "https" && parsed.Scheme != "http") {
		return ""
	}
	if parsed.Scheme == "http" && !isLoopbackMailHost(parsed.Hostname()) {
		return ""
	}
	return parsed.String()
}

func isLoopbackMailHost(host string) bool {
...
func validatedMailBaseURL(raw string) string {
	validated := validatedMailURL(raw)
	if validated == "" {
		return ""
	}
...
func invitePublicURL(baseURL, token string) string {
	base := validatedMailBaseURL(baseURL)
	...
	query := parsed.Query()
	query.Set("invite_token", strings.TrimSpace(token))
...
func buildClientAccessBody(client domain.Client, extraMessage string, artifacts []domain.Artifact, shareLinks []domain.ShareLink, attachmentNotes []string, publicBaseURL string) string {
	...
	if len(shareLinks) > 0 {
		...
		for _, link := range shareLinks {
			...
			if linkURL := shareLinkPublicURL(link, publicBaseURL); linkURL != "" {
				b.WriteString(" url: " + linkURL)
			}
			b.WriteString("\n")
		}
	}
