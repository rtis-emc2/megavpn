...
func (s *Server) runtimePreflight(w nethttp.ResponseWriter, r *nethttp.Request) {
	checks := []runtimePreflightCheck{
		s.databasePreflightCheck(r),
		s.schemaMigrationPreflightCheck(r),
		s.secretStoragePreflightCheck(),
		s.publicBaseURLPreflightCheck(),
		s.controlPlaneTLSPreflightCheck(r),
...
func (s *Server) databasePreflightCheck(r *nethttp.Request) runtimePreflightCheck {
...
func (s *Server) schemaMigrationPreflightCheck(r *nethttp.Request) runtimePreflightCheck {
	latest, applied, err := s.store.SchemaMigrationStatus(r.Context())
	if err != nil {
...
func (s *Server) controlPlaneTLSPreflightCheck(r *nethttp.Request) runtimePreflightCheck {
